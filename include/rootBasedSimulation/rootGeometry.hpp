...
private:
  ...
  std::vector<Leaf> leaves; //Mainly for keeping track of simulation information.

  void buildLists(Turtle* turtle, std::vector<Turtle*>& toDraw, std::vector<Turtle*>& toSeed, int depthCount);
  void getTurtleTreeExtent(const Turtle* turtle, TVector3& minExtent, TVector3& maxExtent, int maxDepth=-1);
  void normalizeTurtlesHeight(std::vector<Turtle*> turtles, double height);
  void recursiveTreeBuild(Turtle* startTurtle, int depthStep, TGeoVolume* parentVolume, TVector3 parentPosition);
  void constructLeaf(const Turtle* endTurtle, TGeoVolume* parentVolume, TVector3 parentPosition);
