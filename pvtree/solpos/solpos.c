...
static long int validate(struct posdata *pdat) {
  ...
  if (pdat->function & L_GEOM) {
    ...
    if ((pdat->hour < 0) || (pdat->hour > 24)) retval |= (1L << S_HOUR_ERROR);
    ...
    if (fabs(pdat->timezone) > 12.0) retval |= (1L << S_TZONE_ERROR);
    if ((pdat->interval < 0) || (pdat->interval > 28800))
      retval |= (1L << S_INTRVL_ERROR);

    /* No absurd locations, please. */
    if (fabs(pdat->longitude) > 180.0) retval |= (1L << S_LON_ERROR);
    if (fabs(pdat->latitude) > 90.0) retval |= (1L << S_LAT_ERROR);
  }
...
static void geometry(struct posdata *pdat) {
  ...
  if (pdat->lmst < 0.) pdat->lmst += 360.0;
...
#define CH_MASK (L_ZENETR)
  ...
  if (tdat->sd < -900.0) /* sd was initialized -999 as flag */
  {
    ...
    if (pdat->function | CD_MASK) tdat->cd = cos(raddeg * pdat->declin);
    if (pdat->function | CH_MASK) tdat->ch = cos(raddeg * pdat->hrang);
    if (pdat->function | CL_MASK) tdat->cl = cos(raddeg * pdat->latitude);
    if (pdat->function | SD_MASK) tdat->sd = sin(raddeg * pdat->declin);
    if (pdat->function | SL_MASK) tdat->sl = sin(raddeg * pdat->latitude);
  }
}
