...
struct Token
{
    ...
    Token(const std::string& nm_value, Type nm_type = UnknownYet)
        : type(nm_type), value(nm_value), next(NULL)
    {}
...
struct OptionData
{
    ...
    _Flags flags;
    Token* token;
    OptionData() : flags(CmdLine_NotExtracted), token(nullptr) {}
};
...
template <class T> inline _Option::Result convert(const std::string& nm_s, T& result, std::ios::fmtflags flags)
{
    std::stringstream ss;
    ss.clear();
    ss.flags(flags);
...
public:
    explicit _OptionTBase(const _OptionTBase<T>& other)
        : _Option(), short_opt(other.short_opt), long_opt(other.long_opt), target(other.target)
    {}

    _OptionTBase(char nm_short_opt, const std::string& nm_long_opt, T& nm_target)
        : short_opt(nm_short_opt), long_opt(nm_long_opt), target(nm_target)
    {}
    ...
    virtual Result operator()(ShortOptions& short_ops, LongOptions& long_ops, Token* /*first*/, std::ios::fmtflags flags) const
    {
        ...
        {
            LongOptions::iterator longit = long_ops.find(long_opt);
            if (longit != long_ops.end())
            {
                longit->second.flags = OptionData::CmdLine_Extracted;
                ret = _assign(longit->second.token, flags, short_ops);
            }
        }
...
public:
    ...
    _OptionT(char nm_short_opt, const std::string& nm_long_opt, T& nm_target)
        : _OptionTBase<T>(nm_short_opt, nm_long_opt, nm_target)
    {}
...
public:
    ...
    _OptionT(char nm_short_opt, const std::string& nm_long_opt, std::vector<T>& nm_target)
        : _OptionTBase<std::vector<T> >(nm_short_opt, nm_long_opt, nm_target)
    {}
};
...
public:
 ...
 _DefValOption(char nm_short_opt, const std::string& nm_long_opt, T& nm_target, const T& nm_default_value)
   : BaseOption(nm_short_opt, nm_long_opt, nm_target), default_value(nm_default_value)
    {}
...
public:
    ...
    _GlobalOption(std::vector<T>& nm_target)
        : target(nm_target)
    {}
};

template <class T>
inline _OptionT<T> Option(char nm_short_opt, const std::string& nm_long_opt, T& nm_target)
{
    return _OptionT<T>(nm_short_opt, nm_long_opt, nm_target);
}
...
public:
    ...
    // WITH long_opt:
    OptionPresent(char nm_short_opt, const std::string& nm_long_opt, bool& nm_present)
        : short_opt(nm_short_opt), long_opt(nm_long_opt), present(&nm_present)
    {}

    OptionPresent(char nm_short_opt, const std::string& nm_long_opt)
        : short_opt(nm_short_opt), long_opt(nm_long_opt), present(NULL)
    {}

    // WITHOUT long_opt:
    OptionPresent(char nm_short_opt, bool& nm_present)
        : short_opt(nm_short_opt), present(&nm_present)
    {}

    OptionPresent(char nm_short_opt)
        : short_opt(nm_short_opt), present(NULL)
    {}

    // WITHOUT short_opt
    OptionPresent(const std::string& nm_long_opt, bool& nm_present)
        : short_opt(_Option::NO_SHORT_OPT), long_opt(nm_long_opt), present(&nm_present)
    {}

    OptionPresent(const std::string& nm_long_opt)
        : short_opt(_Option::NO_SHORT_OPT), long_opt(nm_long_opt), present(NULL)
    {}
protected:
    virtual Result operator()(ShortOptions& short_ops, LongOptions& long_ops, Token* /*first*/, std::ios::fmtflags /*flags*/) const
    {
...
public:
    ...
    void flags(std::ios::fmtflags nm_flags)
    {
        _flags = nm_flags;
    }
