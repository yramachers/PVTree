...
class LeafFactory {
 private:
  ...
  LeafFactory(
      LeafFactory& leafFactory);  //!< Prevent copy construction of instances

  //! Store available leaf constructors by a string
  std::map<std::string, std::function<LeafConstructionInterface*(void)>>
 ...
 public:
   ...
   * @param[in] leafName  Name of the leaf as registered in the constructor
   * definition
...
class LeafFactoryRegistrar {
 public:
  explicit LeafFactoryRegistrar(std::string leafName) {
    ...
    LeafFactory::instance()->registerConstructor(
        leafName, [](void) -> LeafConstructionInterface * { return new T(); });
  }
};
