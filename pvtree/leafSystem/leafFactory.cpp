...
std::shared_ptr<LeafConstructionInterface> LeafFactory::getLeaf(
    std::string leafName) {
  ...
  return std::shared_ptr<LeafConstructionInterface>(
      ((*previousEntry).second)());
}

void LeafFactory::registerConstructor(
    std::string leafName,
  ...
  if (previousEntry != m_factoryFunctionRegistry.end()) {
    throw std::invalid_argument(
        "Cannot record two leaf constructors with the same name = " + leafName);
  }
