...
#  endif
    ...
    public:
        ...
        void optUsage( std::ostream& os, std::size_t indent = 0, std::size_t width = Detail::consoleWidth ) const {
            ...
            for( it = itBegin; it != itEnd; ++it ) {
                Detail::Text currentUsage( it->commands(), Detail::TextAttributes()
					   .setWidth( maxWidth+indent )
					   .setIndent( indent ) );
                Detail::Text desc( it->description, Detail::TextAttributes()
				   .setWidth( width - maxWidth - 3 ) );

                for( std::size_t i = 0; i < (std::max)( currentUsage.size(), desc.size() ); ++i ) {
...
namespace Catch {
    ...
    public:
        ...
        explicit RunContext( Ptr<IConfig const> const& currentConfig, Ptr<IStreamingReporter> const& reporter )
