...
class Config : public SharedImpl<IConfig> {
 ...
 public:
  ...
  void useStream(std::string const& streamName) {
    Stream currentStream = createStream(streamName);
    setStreamBuf(currentStream.streamBuf);
    m_stream.release();
...
// NOTE: std::auto_ptr is deprecated in c++11/c++0x
  ...
  class ArgBuilder {
   public:
    ...
    ArgBuilder& detail(std::string const& currentDetail) {
      m_arg->detail = currentDetail;
      return *this;
    }
 ...
 public:
  ...
  void optUsage(std::ostream& os, std::size_t indent = 0,
    ...
    for (it = itBegin; it != itEnd; ++it) {
      ...
      for (std::size_t i = 0; i < (std::max)(currentUsage.size(), desc.size());
           ++i) {
        std::string usageCol = i < currentUsage.size() ? currentUsage[i] : "";
        os << usageCol;
...
inline Clara::CommandLine<ConfigData> makeCommandLineParser() {
  ...
  cli["-x"]["--abortx"].describe("abort after x failures").bind(&abortAfterX,
                                                                "no. failures");
  ...
  cli["--force-colour"].describe("force colourised output").bind(
      &ConfigData::forceColour);
...
class RunContext : public IResultCapture, public IRunner {
 ...
 public:
  explicit RunContext(Ptr<IConfig const> const& currentConfig,
                      Ptr<IStreamingReporter> const& reporter)
      : m_runInfo(currentConfig->name()),
