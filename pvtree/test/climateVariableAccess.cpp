...
TEST_CASE( "climate/climateFactory", "[climate]" ) {
  ...
  // Check the interpolation values being retrieved have not changed signficantly
  int checkPrecision  = 10;
  CHECK( almost_equal( (float) climate->getInterpolatedValue("2 metre temperature",testTime), 283.5870361328f, checkPrecision) );
  CHECK( almost_equal( (float) climate->getInterpolatedValue("Total column water",testTime),  16.9262218475f,  checkPrecision) );
  CHECK( almost_equal( (float) climate->getInterpolatedValue("Surface pressure",testTime),    100382.5f,       checkPrecision) );
  CHECK( almost_equal( (float) climate->getInterpolatedValue("Total cloud cover",testTime),   0.952015791088f, checkPrecision) );
  CHECK( almost_equal( (float) climate->getInterpolatedValue("Total column ozone",testTime),  0.0066255889833f,checkPrecision) );

  // Check the climate data directly
  std::vector<std::shared_ptr<const ClimateData>> directClimateData = climate->getData();
  ...
  if (directClimateData.size() > 1u) {
    for (unsigned int climateIndex=1; climateIndex<directClimateData.size(); climateIndex++){
      if ( directClimateData[climateIndex]->getTime() < directClimateData[climateIndex-1]->getTime() ){
	timeOrdered = false;
	break;
      }
    }
