...
double LeafConstruction::getSensitiveSurfaceArea() { return m_leafArea; }
...
G4VPhysicalVolume* LeafConstruction::Construct() {
  ...
  G4ThreeVector totalMaximums(
      convertVector(m_completeLeaf[0]->getVertex(0)->getPosition()));
  G4ThreeVector totalMinimums(
      convertVector(m_completeLeaf[0]->getVertex(0)->getPosition()));
  ...
  G4LogicalVolume* leafLogicalVolume =
      new G4LogicalVolume(m_leafSolid, sensitiveMaterial, "Leaf");
  leafLogicalVolume->SetVisAttributes(m_leafVisualAttributes);
  new G4LogicalSkinSurface("LeafSkin", leafLogicalVolume,
                           sensitiveOpticalSurface);
  ...
  new G4PVPlacement(identityTransform, leafLogicalVolume, "Leaf",
                    m_worldLogicalVolume, false, 0);

  return worldPhysicalVolume;
}

G4LogicalVolume* LeafConstruction::constructForTree(
    std::shared_ptr<LeafConstructionInterface> leafConstructor,
    Turtle* initialTurtle) {
  m_leafConstructor = leafConstructor;
  m_initialTurtle = initialTurtle;
...
void LeafConstruction::iterateLSystem() {
  ...
  int leafIterationNumber =
      m_leafConstructor->getIntegerParameter("iterationNumber");
...
void LeafConstruction::generateSurface() {
  ...
  // Also don't actually want to keep the turtles (they are not important in
  // this case).
...
void LeafConstruction::getExtentForTree(
    std::shared_ptr<LeafConstructionInterface> leafConstructor,
...
/ unique vertices.
*/
std::vector<std::shared_ptr<Vertex>> LeafConstruction::mergeVertices(
...
void LeafConstruction::solidifyLeaf() {
  ...
  // Need to have equivalent vertices for original surface (otherwise normals
  // will vary!)
  std::vector<std::shared_ptr<Vertex>> uniqueVertices =
      mergeVertices(secondarySurface);
  std::vector<std::shared_ptr<Vertex>> surfaceUniqueVertices =
      mergeVertices(m_leafSurface);

  // Extrapolate surface along the vertex normals
  for (unsigned int v = 0; v < uniqueVertices.size(); v++) {
    TVector3 secondaryPosition =
        uniqueVertices[v]->getPosition() +
        surfaceUniqueVertices[v]->getNormal() * (-1.0 * thickness);
  ...
  for (unsigned int e = 0; e < uniqueEdges.size(); e++) {
    ...
    TVector3 extrapolatedPosition1 =
        uniqueEdges[e].first->getPosition() +
        uniqueEdges[e].first->getNormal() * (-1.0 * thickness);
    TVector3 extrapolatedPosition2 =
        uniqueEdges[e].second->getPosition() +
        uniqueEdges[e].second->getNormal() * (-1.0 * thickness);

    Polygon* face1 = new Polygon();
    face1->addVertex(std::shared_ptr<Vertex>(
        new Vertex(uniqueEdges[e].first->getPosition())));
    face1->addVertex(
        std::shared_ptr<Vertex>(new Vertex(extrapolatedPosition1)));
    face1->addVertex(std::shared_ptr<Vertex>(
        new Vertex(uniqueEdges[e].second->getPosition())));
    ...
    face2->addVertex(std::shared_ptr<Vertex>(
        new Vertex(uniqueEdges[e].second->getPosition())));
    face2->addVertex(
        std::shared_ptr<Vertex>(new Vertex(extrapolatedPosition1)));
    face2->addVertex(
        std::shared_ptr<Vertex>(new Vertex(extrapolatedPosition2)));
  ...
  m_completeLeaf.insert(m_completeLeaf.end(), m_leafSurface.begin(),
                        m_leafSurface.end());
  m_completeLeaf.insert(m_completeLeaf.end(), secondarySurface.begin(),
                        secondarySurface.end());
  m_completeLeaf.insert(m_completeLeaf.end(), edgeSurface.begin(),
                        edgeSurface.end());
