...
Plenoptic3D::Plenoptic3D() : m_histogram(nullptr), 
  m_surfaceScale(1.0),
...
Plenoptic3D::Plenoptic3D(int azimuthBinNumber, 
			 int elevationBinNumber, 
			 int energyBinNumber) : m_histogram(nullptr), 
						m_surfaceScale(1.0),
						m_hasOptimizedSampling(false),
						m_histogramDefinitionChanged(true),
						m_seedChanged(true),
...
void Plenoptic3D::setBinning(Axis selectedAxis, int binNumber) {
  setBinning(selectedAxis, binNumber, m_axisMinimumAllowed[selectedAxis], m_axisMaximumAllowed[selectedAxis]);
}

void Plenoptic3D::setBinning(Axis selectedAxis, int binNumber, double minimumValue, double maximumValue) {
...
void Plenoptic3D::setBinning(Axis selectedAxis, std::vector<double> binLowEdges) {
  m_binLowEdges[selectedAxis] = binLowEdges;
  m_histogramDefinitionChanged = true;
}

void Plenoptic3D::fill(double azimuth, double elevation, double energy, double weight) {
...
void Plenoptic3D::estimateSurfaceFluxes() {
  ...
  if (m_surfaceAreas.size() != m_surfaceNormals.size()) {
    std::cerr << "Number of surface areas is different from the number of normal vectors" << std::endl;
    throw;
  }

  m_surfaceFlux.clear();

  for (unsigned int surfaceIndex=0; surfaceIndex<m_surfaceAreas.size(); surfaceIndex++) {
    m_surfaceFlux.push_back( calculateSurfaceFlux(m_surfaceAreas[surfaceIndex], m_surfaceNormals[surfaceIndex]) );
  }

  m_hasOptimizedSampling = true;
}

void Plenoptic3D::generate(G4Event* event, WeightedParticleGun* particleGun, unsigned int number) {
  ...
  // Need to keep track of the number of particles created per surface for weighting.
  ...
  std::piecewise_constant_distribution<> surfaceElementDistribution( m_surfaceID.begin(), 
								     m_surfaceID.end(), 
								     m_surfaceFlux.begin() );
  ...
  while (successNumber < number) {
    ...
    if (selectedSurface < 0 || selectedSurface >= (int)(m_surfaceFlux.size()) ) {
      std::cerr << "Selected surface number " << selectedSurface << " which does not exist!" << std::endl;  
      throw;
    }
    ...
    currentLightVector = currentLightVector * -1.0; // invert direction ( sun -> leaf )

    // Check that the angle with respect to the surface normal is < pi/2
    // (passing through the surface in the right direction)
    double angleBetweenVectors = currentLightVector.Angle(m_surfaceNormals[selectedSurface]);
    ...
    currentStartingPosition.SetX(m_surfaceScale * getOrderedFractionalValue(posFractionDist(*m_generator), 
									    m_surfaces[selectedSurface].first.X(),
									    m_surfaces[selectedSurface].second.X() ) );
    currentStartingPosition.SetY(m_surfaceScale * getOrderedFractionalValue(posFractionDist(*m_generator), 
									    m_surfaces[selectedSurface].first.Y(),
									    m_surfaces[selectedSurface].second.Y() ) );
    currentStartingPosition.SetZ(m_surfaceScale * getOrderedFractionalValue(posFractionDist(*m_generator), 
    ...
    // weight = totalHistogramWeight * |cos(DeltaAngle)| * area of surface / total number of particles tried
    // Probably should triple check this!
  ...
  for (unsigned int p=0; p<photonStartingPositions.size(); p++) {
    ...
    particleGun->SetParticleMomentumDirection(G4ThreeVector(photonDirections[p].X(),
							    photonDirections[p].Y(),
							    photonDirections[p].Z() ));

    // Set the initial position
    particleGun->SetParticlePosition(G4ThreeVector(photonStartingPositions[p].X(), 
						   photonStartingPositions[p].Y(), 
    ...
    particleGun->GenerateWeightedPrimaryVertex(event, 
...
TH2D* Plenoptic3D::getEnergyProjectedHistogram() const {
  ...
  std::cerr << "No plenoptic histogram created. Therefore nothing can be projected." << std::endl;
  return nullptr;
}
...
void Plenoptic3D::append(const Plenoptic3D& source) {
  ...
  for (auto& axisType : {AZIMUTH, ELEVATION, ENERGY}){
    ...
    if ( m_binLowEdges[axisType].size() != source.m_binLowEdges.at(axisType).size() ) {
      hasDifferentBinning = true;
      break;
    }

    if ( ! std::equal(m_binLowEdges[axisType].begin(), m_binLowEdges[axisType].end(), source.m_binLowEdges.at(axisType).cbegin(), 
		      [precision](double x, double y) -> bool { return almost_equal(x, y, precision); } ) ) {
      hasDifferentBinning = true;
      break;
    }
  ...
  if (hasDifferentBinning) {
    std::cerr << "Plenoptic3D cannot be appended as the binning is different" << std::endl;
    throw;
  }
  ...
  if (!additionResult) {
    std::cerr << "Plenoptic3D histogram not added succesfully for some reason." << std::endl;
    throw;
  }
...
void Plenoptic3D::setSurfaceGeometry() {
  ...
  m_surfaces.clear();
  m_surfaces.push_back( std::make_pair(TVector3(-0.5, -0.5,  1.0), TVector3( 0.5,  0.5,  1.0)) );
  m_surfaces.push_back( std::make_pair(TVector3(-0.5, -0.5,  0.0), TVector3( 0.5, -0.5,  1.0)) );
  m_surfaces.push_back( std::make_pair(TVector3(-0.5,  0.5,  0.0), TVector3( 0.5,  0.5,  1.0)) );
  m_surfaces.push_back( std::make_pair(TVector3(-0.5, -0.5,  0.0), TVector3(-0.5,  0.5,  1.0)) );
  m_surfaces.push_back( std::make_pair(TVector3( 0.5, -0.5,  0.0), TVector3( 0.5,  0.5,  1.0)) );
...
void Plenoptic3D::constructHistogram() {
  ...
  std::copy(m_binLowEdges[AZIMUTH].begin(), m_binLowEdges[AZIMUTH].end(), azimuthBinLowEdgesArray);

  double* elevationBinLowEdgesArray = new double[m_binLowEdges[ELEVATION].size()];
  std::copy(m_binLowEdges[ELEVATION].begin(), m_binLowEdges[ELEVATION].end(), elevationBinLowEdgesArray);

  double* energyBinLowEdgesArray = new double[m_binLowEdges[ENERGY].size()];
  std::copy(m_binLowEdges[ENERGY].begin(), m_binLowEdges[ENERGY].end(), energyBinLowEdgesArray);

  m_histogram = new TH3D("plenoptic3D", "plenoptic3D", 
			 m_binLowEdges[AZIMUTH].size()-1,   azimuthBinLowEdgesArray,
			 m_binLowEdges[ELEVATION].size()-1, elevationBinLowEdgesArray,
			 m_binLowEdges[ENERGY].size()-1,    energyBinLowEdgesArray);
...
double Plenoptic3D::calculateSurfaceFlux(double surfaceArea, TVector3 normal) const {
  ...
  for (int bx=1; bx<m_histogram->GetNbinsX()+1; bx++) {
	...
	continue;
      ...
      double angleFactor = std::fabs(std::cos(angleBetweenVectors)) * surfaceArea;
...
double Plenoptic3D::calculateSurfaceArea(std::pair<TVector3, TVector3> surface) const {
...
double Plenoptic3D::getOrderedFractionalValue(double fraction, double valueA, double valueB) const {
...
void Plenoptic3D::setRandomPhotonPolarisation(WeightedParticleGun* particleGun) const {
