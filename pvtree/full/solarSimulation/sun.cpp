...
void Sun::updateEnvironment() {
  ...
  if (getClimateOption(TEMPERATURE) ){
    ...
    double currentTemperature  = ClimateFactory::instance()->getClimate()->getInterpolatedValue("2 metre temperature", currentTime);
    this->m_solarPositionData.temp = currentTemperature - CLHEP::STP_Temperature; // Need to convert from Kelvin.
  }

  if (getClimateOption(PRESSURE)) {
    double currentPressure =     ClimateFactory::instance()->getClimate()->getInterpolatedValue("Surface pressure", currentTime);
    this->m_solarPositionData.press = currentPressure * 0.01; // Pa to hPa.

    // Also update the spectrum factory as well
    SpectrumFactory::instance()->setAtmosphericPressure(currentPressure * 0.01); // Pa to mb
  }

  if (getClimateOption(COLUMNWATER)) {
    double currentPrecipitableWater = ClimateFactory::instance()->getClimate()->getInterpolatedValue("Total column water", currentTime);
    SpectrumFactory::instance()->setPrecipitableWater( currentPrecipitableWater / ( (gram/cm2) * (m2/kilogram) ) ); // kg/m^2 to g/cm^2
  }
  ...
  if (getClimateOption(COLUMNOZONE)) {
    ...
    double ozoneDensity = ClimateFactory::instance()->getClimate()->getInterpolatedValue("Total column ozone", currentTime);
    double ozoneAbundance = ( (ozoneDensity/oxygen3Mass)/loschmidtConstant )*100.0; //atm-cm 
    SpectrumFactory::instance()->setOzoneAbundance(ozoneAbundance);
  }

  // Also need the total cloud cover
  if (getClimateOption(CLOUDCOVER)) {
    double currentCloudCover = ClimateFactory::instance()->getClimate()->getInterpolatedValue("Total cloud cover", currentTime);
...
void Sun::updateSolarPosition() {
  ...
  factory->setSolarPositionWithElevationAzimuth(this->getElevationAngle()*(180.0/M_PI), 
						this->getAzimuthalAngle()*(180.0/M_PI));
}

Sun::Sun(LocationDetails deviceLocation) : m_recalculateEnvironment(true), 
  ...
  this->m_solarPositionData.latitude =  m_deviceLocation.getLatitude();  /* in DECIMAL DEGREES, not Deg/Min/Sec */
  ...
  this->m_solarPositionData.timezone  = 0.0;   /* = GMT; DO NOT ADJUST FOR DAYLIGHT SAVINGS TIME. */
...
Sun::~Sun() {
  //I dont think anything is necessary here. At the very least their test did not clean anything up.
}
...
TVector3 Sun::getLightVector() {
  ...
  double azimuth = this->m_solarPositionData.azim; // N=0.0, E=90.0, S=180.0, W=270.0
  double elevation = this->m_solarPositionData.elevref; //Refracted elevation angle of the sun
...
double Sun::getIrradiance() {
  ...
  return this->m_solarPositionData.etrn; //Returning the ET direct normal solar irradiance
}
...
double Sun::getSunsetTime() {
  ...
  return this->m_solarPositionData.ssetr; //Returning the minutes since midnight for sunset time (no refraction)
}
...
double Sun::getSunriseTime() {
  ...
  return this->m_solarPositionData.sretr; //Returning the minutes since midnight for sunrise time (no refraction)
}
