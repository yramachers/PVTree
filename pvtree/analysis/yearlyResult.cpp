...
void YearlyResult::setTree(TreeConstructionInterface* tree) { m_tree = tree; }

void YearlyResult::setLeaf(LeafConstructionInterface* leaf) { m_leaf = leaf; }
...
TreeConstructionInterface* YearlyResult::getTree() { return m_tree; }

LeafConstructionInterface* YearlyResult::getLeaf() { return m_leaf; }

std::vector<double> YearlyResult::getEnergyDeposited() const {
  return m_energyDeposited;
}

double YearlyResult::getEnergyDeposited(
    time_t time,
  ...
  auto nextTimeIterator =
      std::lower_bound(begin(dayTimes), end(dayTimes), time);
...
double YearlyResult::getEnergyIntegral(
  ...
  if (startTime > endTime) {
    std::cerr << "WARNING: Requested energy integral where start time is after "
                 "end time." << std::endl;
    return 0.0;
  }
