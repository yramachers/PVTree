...
unsigned int Polygon::size() { return m_vertices.size(); }
...
void Polygon::replaceVertex(std::shared_ptr<Vertex> originalVertex,
  ...
  std::replace(m_vertices.begin(), m_vertices.end(), originalVertex,
               replacementVertex);
}
...
double Polygon::getArea() {
  if (this->size() != 3) {
    std::cerr
        << "Polygon does not handle the non triangle case for area evaluation"
        << std::endl;
    throw;
  }
