...
void Vertex::setPosition(TVector3 position) { m_position = position; }

TVector3 Vertex::getPosition() const { return m_position; }
...
TVector3 Vertex::getNormal() {
  ...
  // Always return a unit vector (also don't need to then divide by the number
  // of faces!)
...
TVector3 Vertex::getEdgeNormal(std::shared_ptr<Vertex> otherVertex) {
  ...
  for (Polygon* testFace : m_containingFaces) {
    auto otherVertexFace =
        std::find(otherVertex->m_containingFaces.begin(),
                  otherVertex->m_containingFaces.end(), testFace);
  ...
  // Use cross product to get the edge face normal vector (or at least something
  // vaguelly similar)
  TVector3 candidateEdgeNormal = commonFaceNormal.Cross(edgeVector).Unit();

  // Check candidate direction is pointing in the right direction (i.e. not
  // towards the centre of the face!)
...
void Vertex::registerFace(Polygon* face) {
  ...
  auto facePointer =
      std::find(m_containingFaces.begin(), m_containingFaces.end(), face);
