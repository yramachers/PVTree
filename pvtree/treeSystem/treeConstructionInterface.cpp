...
/*! \brief Print all the parameters stored in the constructor and also their
 *ranges.
 *
 * Useful for making quick checks that parameters are reasonable.
 */
void TreeConstructionInterface::print(std::ostream& os /*= std::cout*/) {
  ...
  // Print header
  unsigned int totalTableWidth =
      minimumParameterColumnSize + 3 * valueColumnSize + 13u;
  os << " " << std::setfill('-') << std::setw(totalTableWidth) << ""
  ...
  for (auto& parameterName : m_doubleParameterNames) {
    os << " | " << std::setw(minimumParameterColumnSize) << parameterName
       << " : " << std::setw(valueColumnSize)
       << getDoubleParameter(parameterName) << " | "
       << std::setw(valueColumnSize) << getDoubleRange(parameterName).first
       << " | " << std::setw(valueColumnSize)
       << getDoubleRange(parameterName).second << " |" << std::endl;
  }
  ...
  for (auto& parameterName : m_integerParameterNames) {
    os << " | " << std::setw(minimumParameterColumnSize) << parameterName
       << " : " << std::setw(valueColumnSize)
       << getIntegerParameter(parameterName) << " | "
       << std::setw(valueColumnSize) << getIntegerRange(parameterName).first
       << " | " << std::setw(valueColumnSize)
       << getIntegerRange(parameterName).second << " |" << std::endl;
  }
  os << " " << std::setfill('-') << std::setw(totalTableWidth) << ""
     << std::setfill(' ') << std::endl;
}

/*! \brief Check that two TreeConstructionInterfaces have identical
 *         properties
 */
bool TreeConstructionInterface::operator==(
    const TreeConstructionInterface& right) {
  ...
  for (auto leftName : this->getDoubleParameterNames()) {
    if (!almost_equal(this->getDoubleParameter(leftName),
                      right.getDoubleParameter(leftName),
                      equalityPrecisionFactor)) {
      return false;
    }

    if (!almost_equal(this->getDoubleRange(leftName).first,
                      right.getDoubleRange(leftName).first,
                      equalityPrecisionFactor)) {
      return false;
    }

    if (!almost_equal(this->getDoubleRange(leftName).second,
                      right.getDoubleRange(leftName).second,
                      equalityPrecisionFactor)) {
      return false;
    }
  }
  ...
  for (auto leftName : this->getIntegerParameterNames()) {
    if (this->getIntegerParameter(leftName) !=
        right.getIntegerParameter(leftName)) {
      return false;
    }

    if (this->getIntegerRange(leftName).first !=
        right.getIntegerRange(leftName).first) {
      return false;
    }

    if (this->getIntegerRange(leftName).second !=
        right.getIntegerRange(leftName).second) {
      return false;
    }
  }
...
/*! \brief Check that two TreeConstructionInterfaces do not have
...
/*! \brief For all the parameters specified in the tree randomally choose
 ...
 */
void TreeConstructionInterface::randomizeParameters(int seed) {
  std::default_random_engine randomEngine(seed);

  // Iterate over all double parameters
  for (unsigned int s = 0u; s < m_doubleParameterNames.size(); s++) {
    std::uniform_real_distribution<double> distribution(
        m_doubleParameterRanges[m_doubleParameterNames[s]].first,
  ...
  for (unsigned int s = 0u; s < m_integerParameterNames.size(); s++) {
    std::uniform_int_distribution<int> distribution(
        m_integerParameterRanges[m_integerParameterNames[s]].first,
        m_integerParameterRanges[m_integerParameterNames[s]].second);

    m_integerParameters[m_integerParameterNames[s]] =
 ...
 * \throws std::invalid_argument   If a parameter with the given name and of any
 *type doesn't exist.
 */
void TreeConstructionInterface::randomizeParameter(int seed, std::string name) {
  ...
  if (m_doubleParameterRanges.find(name) != m_doubleParameterRanges.end()) {
    std::uniform_real_distribution<double> distribution(
        m_doubleParameterRanges[name].first,
  ...
  if (m_integerParameterRanges.find(name) != m_integerParameterRanges.end()) {
    std::uniform_int_distribution<int> distribution(
        m_integerParameterRanges[name].first,
  ...
  if (!parameterExists) {
    // Failed to find any parameter to randomize, throw an exception because
    // this is not a good sign
    throw std::invalid_argument("Parameter \"" + name + "\" does not exist");
  }
}

/*! \brief Set the allowed range of a double parameter. If the current value
 *lies
 * above the new maximum then it is set to the maximum and similarly if the
 *current
 ...
 */
void TreeConstructionInterface::setRandomParameterRange(std::string name,
                                                        double minValue,
                                                        double maxValue) {
  ...
  if (mapIterator == m_doubleParameterRanges.end()) {
    ...
    m_doubleParameterRanges[name] =
        std::pair<double, double>(minValue, maxValue);
...
/*! \brief Set the allowed range of an integer parameter. If the current value
 *lies
 * above the new maximum then it is set to the maximum and similarly if the
 *current
 * value is below the minimum it is set to the minimum value.
...
/*! \brief Set a double parameter value. If the parameter did not previously
 *exist
 * it is added and a random range is defined. The initial random range has both
 *the
 ...
 */
void TreeConstructionInterface::setParameter(std::string name, double value) {
...
/*! \brief Set an integer parameter value. If the parameter did not previously
 *exist
 * it is added and a random range is defined. The initial random range has both
 *the
 * minimum and maximum set to the initial value.
 ...
 * @param[in] value     The new value for the parameter.
 */
void TreeConstructionInterface::setParameter(std::string name, int value) {
 ...
 * \throws std::invalid_argument   If a parameter with the given name and type
 *doesn't exist.
 */
double TreeConstructionInterface::getDoubleParameter(std::string name) const {
  auto mapIterator = m_doubleParameters.find(name);
 ...
 *doesn't exist.
 */
int TreeConstructionInterface::getIntegerParameter(std::string name) const {
  auto mapIterator = m_integerParameters.find(name);
 ...
 *
 * @param[in] name      The name of the parameter which should have its range
 *returned.
 * \throws std::invalid_argument   If a parameter with the given name and type
 *doesn't exist.
 */
std::pair<double, double> TreeConstructionInterface::getDoubleRange(
  ...
  // Failed to find parameter, throw an exception
  throw std::invalid_argument("Parameter range \"" + name +
                              "\" does not exist");
}
 ...
 *doesn't exist.
 */
std::pair<int, int> TreeConstructionInterface::getIntegerRange(
    std::string name) const {
  auto mapIterator = m_integerParameterRanges.find(name);
 ...
 *
 */
std::vector<std::string> TreeConstructionInterface::getDoubleParameterNames()
    const {
 ...
 *
 */
std::vector<std::string> TreeConstructionInterface::getIntegerParameterNames()
    const {
 ...
 *
 */
bool TreeConstructionInterface::openConfigurationFile(std::string fileName,
...
/*! \brief Load tree parameters from configuration file
 *
 */
void TreeConstructionInterface::applyConfigurationFile(
  ...
  try {
    libconfig::Setting& parameters = cfg.lookup("tree.parameters");
    ...
    for (int p = 0; p < numberOfParameters; p++) {
      ...
      if (parameters[p]["value"].getType() == libconfig::Setting::TypeInt) {
        ...
        if (parameters[p].exists("minimum") &&
            parameters[p].exists("maximum")) {
          int minimum, maximum;
...
ClassImp(TreeConstructionInterface)
