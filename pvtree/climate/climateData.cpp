...
ClimateData::ClimateData(
    std::shared_ptr<std::map<std::string, int>> nameToParameterID, time_t time)
    : m_time(time), m_nameToParameterID(nameToParameterID) {}

ClimateData::~ClimateData() {}

void ClimateData::setParameterMapping(
...
double ClimateData::getValue(std::string parameterName) const {
  ...
  std::cerr << "Could not find parameter with name: " << parameterName
            << std::endl;
  throw;
}
...
double ClimateData::getValue(int parameterID) const {
  if (m_parameterValues.find(parameterID) == m_parameterValues.end()) {
    std::cerr << "Could not find parameter with ID: " << parameterID
              << std::endl;
    throw;
  }
...
time_t ClimateData::getTime() const { return m_time; }

void ClimateData::setTime(time_t time) { m_time = time; }
