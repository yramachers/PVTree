...
TGraphAsymmErrors createInterpolationGraph(
    std::string graphName, time_t startTime, time_t endTime, long timeStepSize,
    std::string valueName,
    std::function<double(double)> valueModifictionFunction = [](double v) {
      return v;
    }) {
 ...
 *
 * The valueModificationFunction parameter is a transformation function applied
 *to climate
 * variable, which by default is a function which leaves the climate variable
 *unchanged. Can't
 * use normal doxygen documentation because it doesn't recognise std::function!
 *(old doxygen?)
 *
 ...
 * @param[in] daytimeOnly Check that all climate variables are sampled during
 *the daytime if true.
 * @param[in] sun The object describing the suns behaviour at the test location.
 */
TH2D createCandlePlot(std::string plotName, time_t startTime, time_t endTime,
                      long sampleStepSize, std::string valueName,
                      bool daytimeOnly, Sun& sun,
                      std::function<double(double)> valueModificationFunction =
                          [](double v) { return v; }) {
  ...
  double yAxisExtension =
      std::fabs(yAxisMax - yAxisMin) * yAxisExtensionFraction;
  yAxisMin -= yAxisExtension;
  yAxisMax += yAxisExtension;

  // Replace x axis with variable bin sizes based upon actual months start and
  // stop
  // TH2D(const char* name, const char* title, Int_t nbinsx, const Double_t*
  // xbins, Int_t nbinsy, Double_t ylow, Double_t yup)
  std::vector<double> monthlyBinLowEdgeVector =
      createMonthlyBinEdges(startTime, endTime);

  double* monthlyBinLowEdgeArray = new double[monthlyBinLowEdgeVector.size()];
  std::copy(monthlyBinLowEdgeVector.begin(), monthlyBinLowEdgeVector.end(),
            monthlyBinLowEdgeArray);

  // Create the 2D histogram
  int yAxisBinNumber = 20;
  TH2D candlePlot(plotName.c_str(), plotName.c_str(),
                  monthlyBinLowEdgeVector.size() - 1, monthlyBinLowEdgeArray,
                  yAxisBinNumber, yAxisMin, yAxisMax);
  for (long t = 0; t < sampleStepNumber; t++) {
    time_t currentTime = startTime + t * sampleStepSize;
...
template <typename T>
void createStackedCanvas(std::string canvasName, std::vector<T> plots,
                         std::vector<std::string> options,
                         std::vector<int> colours, std::string xAxisTitle,
  ...
  for (unsigned int p = 0; p < plots.size(); p++) {
    ...
    if (p == plots.size() - 1) {
      ...
      TGaxis* xAxis = new TGaxis(
          ...
          plots[p].GetYaxis()->GetXmin(),
          plots[p].GetXaxis()->GetXmin() + absolutePaddingSize,
          plots[p].GetXaxis()->GetXmax() - absolutePaddingSize, 506, "tS");
...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<const ClimateData>> extractedData =
      ClimateFactory::instance()->getClimate()->getData();
  ...
  for (unsigned int d = 0; d < extractedData.size(); d++) {
    ...
    temperatureGraph.SetPoint(
        nextPointIndex, currentTime,
        extractedData[d]->getValue("2 metre temperature") -
            CLHEP::STP_Temperature);
    totalColumnWaterGraph.SetPoint(
        nextPointIndex, currentTime,
        extractedData[d]->getValue("Total column water"));
    surfacePressureGraph.SetPoint(
        nextPointIndex, currentTime,
        extractedData[d]->getValue("Surface pressure") / 1000.0);
    totalCloudCoverGraph.SetPoint(
        nextPointIndex, currentTime,
        extractedData[d]->getValue("Total cloud cover"));
    totalColumnOzoneGraph.SetPoint(
        nextPointIndex, currentTime,
        extractedData[d]->getValue("Total column ozone") * 1000.0);
  }

  // Construct some interpolation graphs
  TGraphAsymmErrors temperatureInterpolGraph = createInterpolationGraph(
      "interpolatedTemperature", interpolationStartTime, interpolationEndTime,
      sampleStepSize, "2 metre temperature",
      [](double v) { return v - CLHEP::STP_Temperature; });

  TGraphAsymmErrors totalColumnWaterInterpolGraph = createInterpolationGraph(
      "interpolatedTotalColumnWater", interpolationStartTime,
      interpolationEndTime, sampleStepSize, "Total column water");

  TGraphAsymmErrors surfacePressureInterpolGraph = createInterpolationGraph(
      "interpolatedSurfacePressure", interpolationStartTime,
      interpolationEndTime, sampleStepSize, "Surface pressure",
      [](double v) { return v / 1000.0; });

  TGraphAsymmErrors totalCloudCoverInterpolGraph = createInterpolationGraph(
      "interpolatedTotalCloudCover", interpolationStartTime,
      interpolationEndTime, sampleStepSize, "Total cloud cover");

  TGraphAsymmErrors totalColumnOzoneInterpolGraph = createInterpolationGraph(
      "interpolatedTotalColumnOzone", interpolationStartTime,
      interpolationEndTime, sampleStepSize, "Total column ozone",
      [](double v) { return v * 1000.0; });
  // Create some candle plots
  TH2D temperatureCandlePlot = createCandlePlot(
      "candleTemperature", interpolationStartTime, interpolationEndTime,
      sampleStepSize, "2 metre temperature", daytimeOnly, sun,
      [](double v) { return v - CLHEP::STP_Temperature; });

  TH2D totalColumnWaterCandlePlot = createCandlePlot(
      "candleTotalColumnWater", interpolationStartTime, interpolationEndTime,
      sampleStepSize, "Total column water", daytimeOnly, sun);

  TH2D surfacePressureCandlePlot =
      createCandlePlot("candleSurfacePressure", interpolationStartTime,
                       interpolationEndTime, sampleStepSize, "Surface pressure",
                       daytimeOnly, sun, [](double v) { return v / 1000.0; });

  TH2D totalCloudCoverCandlePlot = createCandlePlot(
      "candleTotalCloudCover", interpolationStartTime, interpolationEndTime,
      sampleStepSize, "Total cloud cover", daytimeOnly, sun);

  TH2D totalColumnOzoneCandlePlot = createCandlePlot(
      "candleTotalColumnOzone", interpolationStartTime, interpolationEndTime,
      sampleStepSize, "Total column ozone", daytimeOnly, sun,
      [](double v) { return v * 1000.0; });
  ...
  createCanvas<TGraphAsymmErrors>(
      "temperatureOverYearCanvas", {temperatureGraph, temperatureInterpolGraph},
      {"AL", "SAMEL"}, {kBlack, kRed}, "Date", "T_{2m} [#circC]");

  createCanvas<TGraphAsymmErrors>(
      "totalColumnWaterOverYearCanvas",
      {totalColumnWaterGraph, totalColumnWaterInterpolGraph}, {"AL", "SAMEL"},
      {kBlack, kRed}, "Date", "TCW [kgm^{-2}]");

  createCanvas<TGraphAsymmErrors>(
      "surfacePressureOverYearCanvas",
      {surfacePressureGraph, surfacePressureInterpolGraph}, {"AL", "SAMEL"},
      {kBlack, kRed}, "Date", "P_{surface} [kPa]");

  createCanvas<TGraphAsymmErrors>(
      "totalCloudCoverOverYearCanvas",
      {totalCloudCoverGraph, totalCloudCoverInterpolGraph}, {"AL", "SAMEL"},
      {kBlack, kRed}, "Date", "C_{total} [0->1]");

  createCanvas<TGraphAsymmErrors>(
      "totalColumnOzoneOverYearCanvas",
      {totalColumnOzoneGraph, totalColumnOzoneInterpolGraph}, {"AL", "SAMEL"},
      {kBlack, kRed}, "Date", "TCO [kgm^{-2}]");

  // Make some canvas versions of the candle plots
  createCanvas<TH2D>("candleTemperatureCanvas", {temperatureCandlePlot},
                     {"CANDLE"}, {kBlack}, "Date", "T_{2m} [#circC]");

  createCanvas<TH2D>("candleTotalColumnWaterCanvas",
                     {totalColumnWaterCandlePlot}, {"CANDLE"}, {kBlack}, "Date",
                     "TCW [kgm^{-2}]");

  createCanvas<TH2D>("candleSurfacePressureCanvas", {surfacePressureCandlePlot},
                     {"CANDLE"}, {kBlack}, "Date", "P_{surface} [Pa]");

  createCanvas<TH2D>("candleTotalCloudCoverCanvas", {totalCloudCoverCandlePlot},
                     {"CANDLE"}, {kBlack}, "Date", "C_{total} [0->1]");

  createCanvas<TH2D>("candleTotalColumnOzoneCanvas",
                     {totalColumnOzoneCandlePlot}, {"CANDLE"}, {kBlack}, "Date",
                     "TCO [gm^{-2}]");

  // Also put all the plots stacked on the same canvas
  // This will save space in the paper if necessary.
  createStackedCanvas<TH2D>(
      "stackedCandleClimateCanvas",
      {temperatureCandlePlot, totalColumnWaterCandlePlot,
       surfacePressureCandlePlot, totalCloudCoverCandlePlot,
       totalColumnOzoneCandlePlot},
      {"A CANDLE", "A CANDLE", "A CANDLE", "A CANDLE", "A CANDLE"},
      {kBlack, kBlack, kBlack, kBlack, kBlack}, "Date",
      {"T_{2m} [#circC]", "TCW [kgm^{-2}]", "P_{surface} [kPa]",
       "C_{total} [0->1]", "TCO [gm^{-2}]"});
