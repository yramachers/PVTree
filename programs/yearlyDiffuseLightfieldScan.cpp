 ...
 *
 * Provides a mechanism to build a diffuse lightfield for a specific period of
 *time.
 *
...
int main(int argc, char** argv) {
  ...
  std::cerr << "THIS WILL PRODUCE GARBAGE RESULTS WHILST USING SMARTS!"
            << std::endl;
  ...
  std::cout << "Splitting plenoptic funciton into " << azimuthBinNumber
            << " azimuth bins " << std::endl;
  std::cout << "\t and " << elevationBinNumber << " elevation bins"
            << std::endl;
  ...
  double azimuthBinStepSize =
      (maximumAzimuth - minimumAzimuth) / azimuthBinNumber;
  double elevationBinStepSize =
      (maximumElevation - minimumElevation) / elevationBinNumber;

  // The lightfield object to fill
  Plenoptic3D lightfield;
  lightfield.setBinning(Plenoptic3D::AZIMUTH, azimuthBinNumber, minimumAzimuth,
                        maximumAzimuth);
  lightfield.setBinning(Plenoptic3D::ELEVATION, elevationBinNumber,
  ...
  for (unsigned int dayIndex = 0; dayIndex < selectedDayTimes.size();
    ...
    for (unsigned int timeIndex = 0; timeIndex < simulationTimeSegments;
      ...
      for (unsigned int azimuthBin = 0; azimuthBin < azimuthBinNumber;
           azimuthBin++) {
        double currentAzimuth = minimumAzimuth +
                                azimuthBin * azimuthBinStepSize +
                                0.5 * azimuthBinStepSize;

        for (unsigned int elevationBin = 0; elevationBin < elevationBinNumber;
             elevationBin++) {
          double currentElevation = minimumElevation +
                                    elevationBin * elevationBinStepSize +
                                    0.5 * elevationBinStepSize;

          // Fill the lightfield
          // Get the tilted irradiance for the current angle
          SpectrumFactory::instance()->setTiltAngles(
              currentElevation * (180.0 / M_PI),
              currentAzimuth * (180.0 / M_PI));
          ...
          std::vector<double> irradiances =
              currentSpectrum->getSMARTSData()["Difuse_tilted_irradiance"];
