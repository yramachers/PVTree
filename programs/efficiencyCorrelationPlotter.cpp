...
class DerivedVariable {
 public:
  ...
  std::function<T(TreeConstructionInterface*, LeafConstructionInterface*)>
      m_evaluate;
  T m_minimumValue;
  T m_maximumValue;

  DerivedVariable(std::string label, std::string prefix,
                  std::function<T(TreeConstructionInterface*,
                                  LeafConstructionInterface*)> evaluateFunction)
      : m_label(label),
        m_persistencePrefix(prefix),
        m_evaluate(evaluateFunction) {}
};
...
class AnalysisSelection {
 public:
  std::string m_label;
  std::function<bool(TreeConstructionInterface*, LeafConstructionInterface*)>
      m_evaluate;

  AnalysisSelection(
      std::string label,
      std::function<bool(TreeConstructionInterface*,
                         LeafConstructionInterface*)> evaluateFunction)
 ...
 *
 */
void getTreeParameters(std::vector<DerivedVariable<double>>& plotParameters,
                       TList* structureList) {
  TreeConstructionInterface* currentSystem =
      ((YearlyResult*)(structureList->At(0)))->getTree();
  std::vector<std::string> parameterNames =
      currentSystem->getDoubleParameterNames();

  for (auto& parameterName : parameterNames) {
    plotParameters.push_back(DerivedVariable<double>(
        parameterName, "tree_" + parameterName,
        [parameterName](TreeConstructionInterface* t,
                        LeafConstructionInterface * /* l*/)
            -> double { return t->getDoubleParameter(parameterName); }));
  }
  ...
  for (auto& parameterName : parameterNames) {
    plotParameters.push_back(DerivedVariable<double>(
        ...
        [parameterName](TreeConstructionInterface* t,
                        LeafConstructionInterface * /* l*/)
            -> double { return t->getIntegerParameter(parameterName); }));
  }
}
 ...
 *
 */
void getLeafParameters(std::vector<DerivedVariable<double>>& plotParameters,
                       TList* structureList) {
  LeafConstructionInterface* currentSystem =
      ((YearlyResult*)(structureList->At(0)))->getLeaf();
  std::vector<std::string> parameterNames =
      currentSystem->getDoubleParameterNames();

  for (auto& parameterName : parameterNames) {
    plotParameters.push_back(DerivedVariable<double>(
        parameterName, "leaf_" + parameterName,
        [parameterName](TreeConstructionInterface* /*t*/,
                        LeafConstructionInterface* l)
            -> double { return l->getDoubleParameter(parameterName); }));
  }
  ...
  for (auto& parameterName : parameterNames) {
    plotParameters.push_back(DerivedVariable<double>(
        ...
        [parameterName](TreeConstructionInterface* /*t*/,
                        LeafConstructionInterface* l)
            -> double { return l->getIntegerParameter(parameterName); }));
  }
}
...
void getParameterRanges(TList* structureList,
  ...
  for (int t = 0; t < structureList->GetSize(); t++) {
    ...
    for (auto& plotParameter : plotParameters) {
      double currentValue =
          plotParameter.m_evaluate(currentTreeSystem, currentLeafSystem);
 ...
 *
 */
void fillHistograms(std::string histogramSetPrefix, TList* structureList,
                    std::vector<AnalysisSelection>& selections,
                    std::vector<DerivedVariable<double>>& plotParameters,
  ...
  TH1D cutFlowHistogram(cutFlowName.c_str(), cutFlowTitle.c_str(),
                        selections.size(), 0, selections.size());

  for (unsigned int s = 0; s < selections.size(); s++) {
    cutFlowHistogram.GetXaxis()->SetBinLabel(s + 1,
                                             selections[s].m_label.c_str());
  }

  // Create the 2D histograms
  std::vector<std::tuple<DerivedVariable<double>, DerivedVariable<double>,
                         TH2D*>> histograms;
  ...
  for (auto& xAxisParameter : plotParameters) {
    ...
    if (almost_equal(xAxisParameter.m_minimumValue,
                     xAxisParameter.m_maximumValue, comparisonPrecision)) {
      currentXBinNumber = 1;
      currentXBinMinimum = xAxisParameter.m_minimumValue -
                           xAxisParameter.m_minimumValue / 1000.0;
      currentXBinMaximum = xAxisParameter.m_maximumValue +
                           xAxisParameter.m_maximumValue / 1000.0;
    }

    for (auto& yAxisParameter : yAxisParameters) {
      std::string histogramName = histogramSetPrefix + "_" +
                                  yAxisParameter.m_persistencePrefix + "_Vs_" +
                                  xAxisParameter.m_persistencePrefix;
      std::string histogramTitle =
          ";" + xAxisParameter.m_label + ";" + yAxisParameter.m_label;
      ...
      if (almost_equal(yAxisParameter.m_minimumValue,
                       yAxisParameter.m_maximumValue, comparisonPrecision)) {
        currentYBinNumber = 1;
        currentYBinMinimum = yAxisParameter.m_minimumValue -
                             yAxisParameter.m_minimumValue / 1000.0;
        currentYBinMaximum = yAxisParameter.m_maximumValue +
                             yAxisParameter.m_maximumValue / 1000.0;
      }

      TH2D* hist =
          new TH2D(histogramName.c_str(), histogramTitle.c_str(),
                   currentXBinNumber, currentXBinMinimum, currentXBinMaximum,
                   currentYBinNumber, currentYBinMinimum, currentYBinMaximum);

      histograms.push_back(
          std::make_tuple(xAxisParameter, yAxisParameter, hist));
    }
  }
  ...
  for (int t = 0; t < structureList->GetSize(); t++) {
    ...
    for (auto& histogramTuple : histograms) {
      double xValue =
          std::get<0>(histogramTuple).m_evaluate(treeSystem, leafSystem);
      double yValue =
          std::get<1>(histogramTuple).m_evaluate(treeSystem, leafSystem);
...
int main(int argc, char** argv) {
  ...
  ops >> GetOpt::Option('o', "outputRootFile", outputFilename,
                        "efficiencyCorrelation.results.root");
  ...
  plotParameters.push_back(
      DerivedVariable<double>("Surface Density", "tree_surfaceDensity",
                              [](TreeConstructionInterface* t,
                                 LeafConstructionInterface * /* l*/) -> double {
        return t->getDoubleParameter("totalEnergy") /
               t->getDoubleParameter("sensitiveArea");
      }));
  plotParameters.push_back(DerivedVariable<double>(
      "Floor Surface Density", "tree_floorSurfaceDensity",
      [](TreeConstructionInterface* t, LeafConstructionInterface * /* l*/)
          -> double {
        return t->getDoubleParameter("totalEnergy") /
               (t->getDoubleParameter("structureXSize") *
                t->getDoubleParameter("structureYSize"));
      }));
  plotParameters.push_back(
      DerivedVariable<double>("Volume Density", "tree_volumeDensity",
                              [](TreeConstructionInterface* t,
                                 LeafConstructionInterface * /* l*/) -> double {
        return t->getDoubleParameter("totalEnergy") /
               (t->getDoubleParameter("structureXSize") *
                t->getDoubleParameter("structureYSize") *
                t->getDoubleParameter("structureZSize"));
      }));
  plotParameters.push_back(
      DerivedVariable<double>("Fractional Energy Error", "tree_fracError",
                              [](TreeConstructionInterface* t,
                                 LeafConstructionInterface * /* l*/) -> double {
        return t->getDoubleParameter("totalEnergyStdDeviation") /
               t->getDoubleParameter("totalEnergy");
      }));
  ...
  // For a total count
  analysis.push_back(AnalysisSelection(
      "Exists",
      [](TreeConstructionInterface* /* t*/, LeafConstructionInterface * /* l*/)
          -> bool { return true; }));
  ...
  analysis.push_back(AnalysisSelection(
      "Has Surface Area",
      [](TreeConstructionInterface* t, LeafConstructionInterface * /* l*/)
          -> bool { return (t->getDoubleParameter("sensitiveArea") != 0.0); }));
  ...
  std::vector<std::string> selectedYAxisParameterNames = {
      "tree_surfaceDensity", "tree_floorSurfaceDensity", "tree_volumeDensity",
      "tree_leafNumber", "tree_fracError"};
  ...
  fillHistograms("", structureList, analysis, plotParameters,
                 selectedYAxisParameters);
