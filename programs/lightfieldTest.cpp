 ...
 */
std::vector<std::tuple<double, double> > generateTestPhotons(
    unsigned int number, double angle) {
  // Create a static histogram to represent a spectrum to be
  // used.
  static bool isSpectrumConstructed = false;
  static TH1D spectrum("GeneratedPhotonSpectrum", "GeneratedPhotonSpectrum", 25,
                       0.0, 100.0);
  ...
  if (!isSpectrumConstructed) {
    ...
    gaussianDistribution->SetParameter(2,
                                       40.0);  // increase standard deviation.
  ...
  if ( angle < minimumActiveAngle || angle > maximumActiveAngle ){
    ...
    scaleFactor *= std::sin( M_PI * (angle -
  minimumActiveAngle)/(maximumActiveAngle - minimumActiveAngle) );
  }
  */
 ...
 *
 * @param[in] plotName Name of the histogram ROOT should create and save to
 *disk.
 * @param[in] irradiances The vector of 2d tuples which contains all the
 *entries.
...
void createConvergencePlot(std::string plotName,
  ...
  for (unsigned int b = 1; b < xAxisBinCentres.size(); b++) {
    double distanceToPreviousBinCentre =
        xAxisBinCentres[b] - xAxisBinCentres[b - 1];

    double currentBinHalfWidth =
        distanceToPreviousBinCentre - (binWidths.back() / 2.0);

    if (currentBinHalfWidth < 0.0) {
      std::cerr << "Can't use negative bin widths. Logic problem in irradiance "
  ...
  double yAxisPadding = yAxisPaddingFraction *
                        std::fabs(maxSummedIrradiance - minSummedIrradiance);
  minSummedIrradiance -= yAxisPadding;
  maxSummedIrradiance += yAxisPadding;

  TH2D candlePlot(plotName.c_str(), plotName.c_str(), binLowEdges.size() - 1,
                  xAxisLowEdgeArray, yAxisBinNumber, minSummedIrradiance,
                  maxSummedIrradiance);
...
void fillPlenopticFunction(Plenoptic1D& plenopticFunction,
                           double sourceMinimumAngle, double sourceMaximumAngle,
                           int photonNumberPerSample, int stepNumber) {
  ...
  for (int s = 0; s < stepNumber; s++) {
    ...
    std::vector<std::tuple<double, double> > photons =
        generateTestPhotons(photonNumberPerSample, currentAngle);
...
double Cross2D(TVector2 v, TVector2 w) { return v.X() * w.Y() - v.Y() * w.X(); }

double Dot2D(TVector2 v, TVector2 w) { return v.X() * w.X() + v.Y() * w.Y(); }
...
bool isIntersecting(TVector2 p, TVector2 r, TVector2 q, TVector2 s) {
  ...
  if (Cross2D(r, s) != 0 && t >= 0.0 && t <= 1.0 && u >= 0.0 && u <= 1.0) {
...
double testGenerationConvergence(std::string testName,
                                 double targetOrientationAngle,
                                 double sourceMinimumAngle,
                                 double sourceMaximumAngle,
                                 int photonNumberPerSample, int seedOffset) {
  ...
  for (unsigned int i = 0; i < sourceStepNumbers.size(); i++) {
    ...
    for (unsigned int a = 0; a < sourceAttempts[i]; a++) {
      ...
      for (int s = 0; s < stepNumber; s++) {
        double currentAngle =
            sourceMinimumAngle + angleStep * s + angleStep * 0.5;

        std::vector<std::tuple<double, double> > photons = generateTestPhotons(
            photonNumberPerSample, currentAngle * (M_PI / 180.0));
        ...
        irradianceSum +=
            energySum * (M_PI / 180.0) * angleStep *
            std::fabs(std::cos((targetOrientationAngle - 90.0 - currentAngle) *
                               (M_PI / 180.0)));
      }
  ...
  createConvergencePlot((testName + std::string("Convergence")).c_str(),
                        targetIrradiances, sourceStepNumbers);
...
void testPlenopticFunction(std::string testName, double targetOrientationAngle,
                           double sourceMinimumAngle, double sourceMaximumAngle,
                           int photonNumberPerSample) {
  ...
  double bestAverageConvergence = testGenerationConvergence(
      testName, targetOrientationAngle, sourceMinimumAngle, sourceMaximumAngle,
      photonNumberPerSample, seedOffset);

  std::cout << std::setw(19) << "Old way sum = " << bestAverageConvergence
            << std::endl;
  ...
  for (unsigned int b = 0; b < regularBinNumber; b++) {
    regularAngularGrid.push_back((regularMin + regularBinSize * b) *
                                 (M_PI / 180.0));
  }
  ...
  fillPlenopticFunction(
      regularPlenopticFunction, sourceMinimumAngle * (M_PI / 180.0),
      sourceMaximumAngle * (M_PI / 180.0), photonNumberPerSample,
      2000);  // step number
  ...
  TVector2 sensitiveStartingPoint(
      -0.5 * std::sin(targetOrientationAngle * (M_PI / 180.0)),
      -0.5 * std::cos(targetOrientationAngle * (M_PI / 180.0)));
  TVector2 sensitiveTrajectory(
      std::sin(targetOrientationAngle * (M_PI / 180.0)),
      std::cos(targetOrientationAngle * (M_PI / 180.0)));
  ...
  for (auto& particle : particles) {
    ...
    TVector2 trajectoryDisplacement(std::sin(std::get<2>(particle)),
                                    std::cos(std::get<2>(particle)));
    trajectoryDisplacement *= lengthScale;

    if (isIntersecting(startingPoint, trajectoryDisplacement,
                       sensitiveStartingPoint, sensitiveTrajectory)) {
  ...
  TH2D ranger(testName.c_str(), testName.c_str(), 30, -10.0, 10.0, 30, -10.0,
              10.0);
  ranger.Draw();
  ...
  for (unsigned int s = 0; s < surfaceGeometry.size() - 1; s++) {
    TLine* line = sceneLine.DrawLine(std::get<0>(surfaceGeometry[s]),
                                     std::get<1>(surfaceGeometry[s]),
                                     std::get<0>(surfaceGeometry[s + 1]),
                                     std::get<1>(surfaceGeometry[s + 1]));
  ...
  for (auto& particle : particles) {
    ...
    TLine* line = particleLine.DrawLine(
        std::get<0>(particle), std::get<1>(particle),
  ...
  for (auto& particle : particles) {
    ...
    if (isIntersecting(startingPoint, trajectoryDisplacement,
                       sensitiveStartingPoint, sensitiveTrajectory)) {
      TLine* line = hitLine.DrawLine(
          std::get<0>(particle), std::get<1>(particle),
          std::get<0>(particle) + lengthScale * std::sin(std::get<2>(particle)),
          std::get<1>(particle) +
              lengthScale * std::cos(std::get<2>(particle)));
      lines.push_back(line);
  ...
  // Draw the target
  lines.push_back(
      sceneLine.DrawLine(sensitiveStartingPoint.X(), sensitiveStartingPoint.Y(),
...
int main(int /*argc*/, char** /*argv*/) {
  ...
  testPlenopticFunction("plenoptic1", targetOrientationAngle,
                        sourceMinimumAngle, sourceMaximumAngle,
  ...
  testPlenopticFunction("plenoptic4", 230.0, 100.0, 180.0,
                        photonNumberPerSample);
  testPlenopticFunction("plenoptic5", 230.0, 150.0, 360.0,
                        photonNumberPerSample);
  testPlenopticFunction("plenoptic6", 230.0, 280.0, 340.0,
                        photonNumberPerSample);
  testPlenopticFunction("plenoptic7", 230.0, 300.0, 320.0,
                        photonNumberPerSample);
