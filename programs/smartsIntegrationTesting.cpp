...
void createCanvas(std::string canvasName, std::string canvasTitle,
  ...
  for (unsigned int h = 0; h < histograms.size(); h++) {
    ...
    histogramName << std::setprecision(3) << histogramSettings[h] << " "
                  << settingUnits;
...
void createProjectedCanvas(std::string canvasName, std::string canvasTitle,
                           TH2D& histogram, std::string xAxisName,
...
int main() {
  ...
  for (double elevation : elevations) {
    ...
    spectralHistograms.push_back(
        spectrum->getHistogram("Direct_normal_irradiance"));
  }
  ...
  for (double waterColumnDensity : waterColumnDensities) {
    double precipitableWater =
        waterColumnDensity /
        ((gram / cm2) * (m2 / kilogram));  // convert to g/cm^2
    factory->setPrecipitableWater(precipitableWater);
    spectrum = factory->getSpectrum();

    // Get a copy of the normal irradiance histogram
    waterVariationHistograms.push_back(
        spectrum->getHistogram("Direct_normal_irradiance"));
  }
  ...
  std::vector<double> atmosphericPressures = {950.0, 970.0, 990.0, 1010.0,
                                              1030.0};  // hPa (or mb...)
  ...
  for (double pressure : atmosphericPressures) {
    ...
    // Get a copy of the normal irradiance histogram
    pressureVariationHistograms.push_back(
        spectrum->getHistogram("Direct_normal_irradiance"));
  }
  ...
  std::vector<double> ozoneDensities = {0.005, 0.007, 0.009, 0.01,
                                        0.012};  // kg m^{-2}
  ...
  for (double ozoneDensity : ozoneDensities) {
    ...
    double abundance =
        ((ozoneDensity / oxygen3Mass) / loschmidtConstant) * 100.0;
    ...
    // Get a copy of the normal irradiance histogram
    ozoneAbundanceHistograms.push_back(
        spectrum->getHistogram("Direct_normal_irradiance"));
  }
  ...
  std::vector<double> temperatures = {-20.0, -10.0, 0.0,
                                      10.0,  20.0,  30.0};  // degrees C
  ...
  for (double temperature : temperatures) {
    ...
    double fixedPrecipitableWater =
        12.0 / ((gram / cm2) * (m2 / kilogram));  // g/cm^2
    ...
    factory->setAtmosphereProperties(temperature, fixedHumidity, fixedTime,
                                     temperature);
    spectrum = factory->getSpectrum();

    // Get a copy of the normal irradiance histogram
    temperatureHistograms.push_back(
        spectrum->getHistogram("Direct_normal_irradiance"));
  }

  // Try varying the relative humidity
  std::vector<double> relativeHumidities = {0.0,  20.0, 40.0,
                                            60.0, 80.0, 100.0};  // %
  ...
  for (double humidity : relativeHumidities) {
    ...
    factory->setPrecipitableWater(fixedPrecipitableWater);
    factory->setGasLoad(fixedGasLoad);
    factory->setAtmosphereProperties(fixedTemperature, humidity, fixedTime,
                                     fixedTemperature);
    spectrum = factory->getSpectrum();

    // Get a copy of the normal irradiance histogram
    humidityHistograms.push_back(
        spectrum->getHistogram("Direct_normal_irradiance"));
  }
  ...
  std::vector<SpectrumFactory::GasLoad> pollutionSelections = {
      SpectrumFactory::PRISTINE, SpectrumFactory::LIGHT_POLLUTION,
      SpectrumFactory::MODERATE_POLLUTION, SpectrumFactory::SEVERE_POLLUTION};
  ...
  for (double tiltedAzimuth : tiltedAzimuthValues) {
    ...
    tiltedDirectHistograms.push_back(
        spectrum->getHistogram("Direct_tilted_irradiance"));
    tiltedDiffuseHistograms.push_back(
        spectrum->getHistogram("Difuse_tilted_irradiance"));
  }
  ...
  TH2D diffuseSkyTotalIrradiance("DiffuseSkyTotalIrradiance", "Aitoff",
                                 tiltedAzimuthValues.size(), -180.0, 180.0,
                                 tiltedElevationValues.size(), 0.0, 90.0);
  ...
  for (double tiltedAzimuth : tiltedAzimuthValues) {
    for (double tiltedElevationValue : tiltedElevationValues) {
      ...
      double diffuseIrradianceSum =
          spectrum->getHistogram("Difuse_tilted_irradiance")->Integral("width");
      diffuseSkyTotalIrradiance.Fill(
          tiltedAzimuth - 180.0, tiltedElevationValue, diffuseIrradianceSum);
    }
  }
  ...
  createCanvas("IrradianceVsElevation", "Irradiance Vs Elevation",
               spectralHistograms, "Wavelength [nm]", "Irradiance [Wm^{-2}]",
               colours, elevations, "degrees");

  // Make the precipitable water variation canvas
  createCanvas("IrradianceVsPrecipitableWater",
               "Irradiance Vs Precipitable Water", waterVariationHistograms,
               "Wavelength [nm]", "Irradiance [Wm^{-2}]", colours,
               waterColumnDensities, "kgm^{-2}");
  ...
  createCanvas("IrradianceVsAtmosphericPressure",
               ...
               pressureVariationHistograms, "Wavelength [nm]",
               "Irradiance [Wm^{-2}]", colours, atmosphericPressures, "hPa");

  // Make the ozone abundance canvas
  createCanvas("IrradianceVsOzoneAbundance", "Irradiance Vs O_{3} Abundance",
               ozoneAbundanceHistograms, "Wavelength [nm]",
               "Irradiance [Wm^{-2}]", colours, ozoneAbundances, "atm-cm");

  // Make the air temperature canvas
  createCanvas("IrradianceVsAirTemperature", "Irradiance Vs Air Temperature",
               temperatureHistograms, "Wavelength [nm]", "Irradiance [Wm^{-2}]",
               colours, temperatures, " degrees C");

  // Make the relative humidity canvas
  createCanvas("IrradianceVsRelativeHumidity",
               "Irradiance Vs Relative Humidity", humidityHistograms,
               "Wavelength [nm]", "Irradiance [Wm^{-2}]", colours,
               relativeHumidities, " %");

  // Make the pollution variation canvas
  createCanvas("IrradianceVsPollution", "Irradiance Vs Pollution",
               pollutionHistograms, "Wavelength [nm]", "Irradiance [Wm^{-2}]",
               colours, pollutionLevels, "");

  // Make the direct tilted irradiance variation canvas
  createCanvas("DirectIrradianceVsSurfaceAzimuth",
               "Direct Tilted Irradiance Vs Azimuth", tiltedDirectHistograms,
               "Wavelength [nm]", "Direct Tilted Irradiance [Wm^{-2}]", colours,
               tiltedAzimuthValues, " deg");

  // Make the diffuse tilted irradiance variation canvas
  createCanvas("DiffuseIrradianceVsSurfaceAzimuth",
               "Diffuse Tilted Irradiance Vs Azimuth", tiltedDiffuseHistograms,
               "Wavelength [nm]", "Diffuse Tilted Irradiance [Wm^{-2}]",
               colours, tiltedAzimuthValues, " deg");

  // Draw the projection
  createProjectedCanvas("DiffuseTiltedAitoff",
                        "Aitoff projection of total irradiance",
                        diffuseSkyTotalIrradiance, "Azimuth", "Elevation");
