...
def changeAxisTime(inputGraph):
    
    for p in range(0, inputGraph.GetN()):
        currentDayNumber = inputGraph.GetX()[p]
        currentY = inputGraph.GetY()[p]

        rootTime = convertToRootTime(2014, currentDayNumber)

        inputGraph.SetPoint(p, rootTime, currentY)
...
example1File = ROOT.TFile("yearlyEnergyPlotter.results.example1.root")
example2File = ROOT.TFile("yearlyEnergyPlotter.results.example2.root")
example1CloudyFile = ROOT.TFile("yearlyEnergyPlotter.results.example1_cloudy.root")
example2CloudyFile = ROOT.TFile("yearlyEnergyPlotter.results.example2_cloudy.root")

example1Graph = example1File.FindObjectAny("averageTreeEnergyDensityGraph")
example2Graph = example2File.FindObjectAny("averageTreeEnergyDensityGraph")
example1CloudyGraph = example1CloudyFile.FindObjectAny("averageTreeEnergyDensityGraph")
example2CloudyGraph = example2CloudyFile.FindObjectAny("averageTreeEnergyDensityGraph")
...
scaleGraph(example2Graph, quantumEfficiency)
scaleGraph(example1CloudyGraph, quantumEfficiency)
scaleGraph(example2CloudyGraph, quantumEfficiency)

# Change the axis to date format
changeAxisTime(example1Graph)
changeAxisTime(example2Graph)
changeAxisTime(example1CloudyGraph)
changeAxisTime(example2CloudyGraph)
...
example2Graph.Draw("SAMELE3")
example1CloudyGraph.Draw("SAMELE3")
example2CloudyGraph.Draw("SAMELE3")
...
example2Graph.SetFillColor(ROOT.kBlue-2)
example2Graph.SetLineColor(ROOT.kBlue-2)

example1CloudyGraph.SetFillColor(ROOT.kRed)
example1CloudyGraph.SetLineColor(ROOT.kRed)

example2CloudyGraph.SetFillColor(ROOT.kBlue)
example2CloudyGraph.SetLineColor(ROOT.kBlue)
...
legend.AddEntry(example1Graph, "Example A", "F")
legend.AddEntry(example2Graph, "Example B", "F")
legend.AddEntry(example1CloudyGraph, "Example A Cloudy", "F")
legend.AddEntry(example2CloudyGraph, "Example B Cloudy", "F")
