...
double LocationDetails::getLongitude() const {
  return m_longitude;
}

double LocationDetails::getLatitude() const {
  return m_latitude;
}

double LocationDetails::getAltitude() const {
  return m_altitude;
}
...
void LocationDetails::extractFile(std::string configFilePath) {
  ...
  if (readCount != 3) {
    throw std::string("Failed to read correct amount of information for LocationDetails.");
  }
}
