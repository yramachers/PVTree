...
LeafTrackerSD::LeafTrackerSD(const G4String& name, const G4String& hitsCollectionName) 
...
void   LeafTrackerSD::Initialize(G4HCofThisEvent* eventHitCollection) {
  ...
  G4int hitCollectionID = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[0]);
...
G4bool LeafTrackerSD::ProcessHits(G4Step* step, G4TouchableHistory* /*history*/) { 
  ...
  hit->setChamberNumber(step->GetPreStepPoint()->GetTouchableHandle()->GetCopyNumber());
