...
LayeredLeafConstruction::LayeredLeafConstruction(std::shared_ptr<LeafConstructionInterface> leafSystem,
...
//     m_frontMaterialName("pv-glass"),
  ...
  m_frontAttributes.SetColour(     G4Colour(0.0,  0.6,  1.0,  1.0) );//Blue (transparent)
...
G4VPhysicalVolume* LayeredLeafConstruction::Construct() {
  ...
  G4TessellatedSolid* envelopeSolid = static_cast<G4TessellatedSolid*>(leafEnvelope->GetSolid());

  G4ThreeVector totalMaximums( envelopeSolid->GetMaxXExtent(), envelopeSolid->GetMaxYExtent(), envelopeSolid->GetMaxZExtent()  );
  G4ThreeVector totalMinimums( envelopeSolid->GetMinXExtent(), envelopeSolid->GetMinYExtent(), envelopeSolid->GetMinZExtent()  );
  ...
  // Applying a scale to the world size. Doesn't matter too greatly in the case of leaf
  ...
  new G4PVPlacement(identityTransform, leafEnvelope,  "LeafEnvelope",  m_worldLogicalVolume,  false, 0);

  return worldPhysicalVolume; 
}

G4LogicalVolume* LayeredLeafConstruction::constructForTree(std::shared_ptr<LeafConstructionInterface> leafSystem,
							   Turtle* initialTurtle) {
  // Make sure to update these settings (may have not been specified in the constructor)
  m_leafSystem = leafSystem;
  m_initialTurtle = initialTurtle;
...
void LayeredLeafConstruction::iterateLSystem() {
  ...
  int leafIterationNumber = m_leafSystem->getIntegerParameter("iterationNumber");
...
std::vector<Polygon*> LayeredLeafConstruction::generateSurface() {
  ...
  for ( auto& condition : m_leafConditions ){
    condition->processTurtles(activeTurtles, retiredTurtles, candidateSurfacePolygons);
  }
  ...
  //Don't actually want to keep the turtles (they are not important in this case).
  ...
  for ( auto& polygon : candidateSurfacePolygons ){
    ...
    for (unsigned int v=1; v<polygon->size(); v++) {
      if ( (polygon->getVertex(0)->getPosition()-polygon->getVertex(v)->getPosition()).Mag() < 0.0001 ) {
	badTriangle = true;
      }
    }
...
G4LogicalVolume* LayeredLeafConstruction::constructLeafLogicialVolume() {
  ...
  if (m_leafSystem == nullptr || m_initialTurtle == nullptr) {
    std::cerr << "Did not construct LayeredLeafConstruction with appropriate information." << std::endl;
    throw;
  }
  ...
  std::vector<Polygon*> frontMesh     = extrapolateSurfaceIntoMesh(initialSystemSurface, 0.5*thickness,  0.03*thickness);
  std::vector<Polygon*> sensitiveMesh = extrapolateSurfaceIntoMesh(initialSystemSurface, 0.03*thickness, 0.0*thickness);
  std::vector<Polygon*> backMesh =      extrapolateSurfaceIntoMesh(initialSystemSurface, 0.0,           -0.5*thickness);
  std::vector<Polygon*> envelopeMesh =  extrapolateSurfaceIntoMesh(initialSystemSurface, 0.5*thickness, -0.5*thickness);
  ...
  G4TessellatedSolid* sensitiveSolid = convertMeshToTessellatedSolid(sensitiveMesh, std::string("LeafSensitiveSolid"));
  G4TessellatedSolid* backSolid      = convertMeshToTessellatedSolid(backMesh,      std::string("LeafBackSolid"));
  G4TessellatedSolid* envelopeSolid  = convertMeshToTessellatedSolid(envelopeMesh,  std::string("LeafEnvelopeSolid"));

  // Get the materials from the factory
  G4Material* frontMaterial =     MaterialFactory::instance()->getMaterial(m_frontMaterialName);
  G4Material* sensitiveMaterial = MaterialFactory::instance()->getMaterial(m_sensitiveMaterialName);
  G4Material* backMaterial =      MaterialFactory::instance()->getMaterial(m_backMaterialName);
  G4Material* airMaterial =       MaterialFactory::instance()->getMaterial(m_airMaterialName);

  // Finally make the leaf geometric sandwich.
  G4LogicalVolume* frontLogicalVolume =     new G4LogicalVolume(frontSolid,     frontMaterial,     "LeafFront");
  G4LogicalVolume* sensitiveLogicalVolume = new G4LogicalVolume(sensitiveSolid, sensitiveMaterial, "LeafSensitive");
  G4LogicalVolume* backLogicalVolume =      new G4LogicalVolume(backSolid,      backMaterial,      "LeafBack");
  G4LogicalVolume* envelopeLogicalVolume =  new G4LogicalVolume(envelopeSolid,  airMaterial,       "LeafEnvelope");
  ...
  G4OpticalSurface* frontOpticalSurface =     MaterialFactory::instance()->getOpticalSurface(m_frontMaterialName);
  G4OpticalSurface* sensitiveOpticalSurface = MaterialFactory::instance()->getOpticalSurface(m_sensitiveMaterialName);
  G4OpticalSurface* backOpticalSurface =      MaterialFactory::instance()->getOpticalSurface(m_backMaterialName);

  // Set the skin surface properties (but not silicon?)
  new G4LogicalSkinSurface("LeafFrontSkin",     frontLogicalVolume,     frontOpticalSurface);
  new G4LogicalSkinSurface("LeafBackSkin",      backLogicalVolume,      backOpticalSurface);

  // Convert the logical volumes into physical volumes embeddeded in the envelope
...
void LayeredLeafConstruction::getExtentForTree(std::shared_ptr<LeafConstructionInterface> leafSystem,
					       Turtle* initialTurtle,
					       G4ThreeVector& minExtent, 
					       G4ThreeVector& maxExtent) {
...
std::vector<std::shared_ptr<Vertex>> LayeredLeafConstruction::mergeVertices(std::vector<Polygon* >& polygons) {
...
std::vector<Polygon*> LayeredLeafConstruction::extrapolateSurfaceIntoMesh(const std::vector<Polygon*>& polygons,
  ...
  std::vector<std::shared_ptr<Vertex>> frontUniqueVertices = mergeVertices(frontSurface);
  std::vector<std::shared_ptr<Vertex>> backUniqueVertices =  mergeVertices(backSurface);
  ...
  for (unsigned int v=0; v<frontUniqueVertices.size(); v++) {
    updatedPositions.push_back( frontUniqueVertices[v]->getPosition() + frontUniqueVertices[v]->getNormal() * frontSurfaceOffsetFactor );
  }
  for (unsigned int v=0; v<frontUniqueVertices.size(); v++) {
  ...
  for (unsigned int v=0; v<backUniqueVertices.size(); v++) {
    updatedPositions.push_back( backUniqueVertices[v]->getPosition() + backUniqueVertices[v]->getNormal() * backSurfaceOffsetFactor );
  }
  for (unsigned int v=0; v<backUniqueVertices.size(); v++) {
  ...
  std::vector<Polygon*> edgeSurface = createEdgeSurface(sourceSurface, 
  ...
  meshSurfacePolygons.insert(meshSurfacePolygons.end(), frontSurface.begin(), frontSurface.end() );
  meshSurfacePolygons.insert(meshSurfacePolygons.end(), backSurface.begin(),  backSurface.end() );
  meshSurfacePolygons.insert(meshSurfacePolygons.end(), edgeSurface.begin(),  edgeSurface.end() );
...
double LayeredLeafConstruction::calculateExtrapolatedSurfaceArea(const std::vector<Polygon*>& polygons,
								 double frontSurfaceOffsetFactor,
								 double backSurfaceOffsetFactor) {
  ...
  // Use the front and edge surfaces to calculate the current 'visible' sensitive area
...
std::vector<Polygon*> LayeredLeafConstruction::createEdgeSurface(const std::vector<Polygon*>& surfacePolygons,
								 double frontSurfaceOffsetFactor,
								 double backSurfaceOffsetFactor) {
  ...
  for ( unsigned int e=0; e<uniqueEdges.size(); e++ ){
    ...
    TVector3 frontPosition1 = uniqueEdges[e].first->getPosition() + uniqueEdges[e].first->getNormal() * frontSurfaceOffsetFactor;
    TVector3 frontPosition2 = uniqueEdges[e].second->getPosition() + uniqueEdges[e].second->getNormal() * frontSurfaceOffsetFactor;
    TVector3 backPosition1  = uniqueEdges[e].first->getPosition() + uniqueEdges[e].first->getNormal() * backSurfaceOffsetFactor;
    TVector3 backPosition2  = uniqueEdges[e].second->getPosition() + uniqueEdges[e].second->getNormal() * backSurfaceOffsetFactor;
...
G4TessellatedSolid* LayeredLeafConstruction::convertMeshToTessellatedSolid(const std::vector<Polygon*>& polygons,
...
void LayeredLeafConstruction::clearPolygonList(std::vector<Polygon*>& polygons) {
