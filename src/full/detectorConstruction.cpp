...
DetectorConstruction::DetectorConstruction(std::shared_ptr<TreeConstructionInterface> treeSystem,
  ...
  m_floorVisualAttributes.SetColour( G4Colour(0.87, 0.87, 0.87, 1.0) ); // Transparent Grey
...
void DetectorConstruction::resetGeometry(std::shared_ptr<TreeConstructionInterface> treeSystem,
					 std::shared_ptr<LeafConstructionInterface> leafSystem) {
  m_treeSystem = treeSystem;
  m_leafSystem = leafSystem;
  resetGeometry();
...
double DetectorConstruction::getXSize() {
  return m_structureXSize;
}

double DetectorConstruction::getYSize() {
  return m_structureYSize;
}

double DetectorConstruction::getZSize() {
  return m_structureZSize;
}
...
G4VPhysicalVolume* DetectorConstruction::Construct() {
  ...
  m_worldPhysicalVolume = new G4PVPlacement(0, G4ThreeVector(0.0, 0.0, 0.0), m_worldLogicalVolume, "World", 0, false, 0);
  ...
  G4Tubs* floorTopSolid = new G4Tubs("FloorTop",
				     pRMin = 0.0,
				     pRMax = std::sqrt(3.0) * 0.95 * boundingRadius,
				     pDz =  topThickness,
				     pSPhi = 0.0,
				     pDPhi = 2.0*M_PI);
  G4Sphere* floorSolid = new G4Sphere("Floor",
				      pRMin = 0.0,
				      pRMax = std::sqrt(3.0) * 0.95 * boundingRadius,
				      pSPhi = 0.0,
				      pDPhi = 2.0*M_PI,
  ...
  G4Material* floorMaterial = MaterialFactory::instance()->getMaterial(m_floorMaterialName);
  G4OpticalSurface* floorOpticalSurface = MaterialFactory::instance()->getOpticalSurface(m_floorMaterialName);

  G4LogicalVolume* floorTopLogicalVolume = new G4LogicalVolume(floorTopSolid, floorMaterial, "FloorTop");
  G4LogicalVolume* floorLogicalVolume = new G4LogicalVolume(floorSolid, floorMaterial, "Floor");
  ...
  /*G4VPhysicalVolume* floorPhysicalVolume =*/ new G4PVPlacement(identityRotation,
								 floorPosition, 
								 floorLogicalVolume, 
								 "Floor", 
  ...
  /*G4VPhysicalVolume* floorPhysicalVolume =*/ new G4PVPlacement(identityTransform, 
								 floorTopLogicalVolume, 
								 "FloorTop", 
								 m_worldLogicalVolume, 
								 false, 
								 0);
  ...
  new G4LogicalSkinSurface("FloorSkin", floorLogicalVolume, floorOpticalSurface);
  new G4LogicalSkinSurface("FloorTopSkin", floorTopLogicalVolume, floorOpticalSurface);

  // Convert the turtles into Geant4 geometry representing the tree structure through a recursive build
  G4ThreeVector startPosition(0.0, 0.0, 0.0);
  for (auto& startingTurtle : startingTurtles) {
    recursiveTreeBuild(startingTurtle, 100, m_worldLogicalVolume, startPosition);
  }
...
void DetectorConstruction::iterateLSystem() {
  ...
  int treeIterationNumber = m_treeSystem->getIntegerParameter("iterationNumber");
...
//! \todo Need to improve the tree extent calculation so it takes into account the width of the trunk/branches. 
void DetectorConstruction::getTurtleTreeExtent(const Turtle* turtle, 
  ...
  auto result = std::minmax({g4StartPosition.x(),g4EndPosition.x(),minExtent.x(),maxExtent.x()});
  minExtent.setX(result.first);
  maxExtent.setX(result.second);

  result = std::minmax({g4StartPosition.y(),g4EndPosition.y(),minExtent.y(),maxExtent.y()});
  minExtent.setY(result.first);
  maxExtent.setY(result.second);

  result = std::minmax({g4StartPosition.z(),g4EndPosition.z(),minExtent.z(),maxExtent.z()});
  ...
  if (turtle->children.size() == 0){
    ...
    m_leafConstructor.getExtentForTree(m_leafSystem, &copiedTurtle, minExtent, maxExtent);
  }
}
...
void DetectorConstruction::recursiveTreeBuild(Turtle* turtle, 
					      int /*depthStep*/, 
					      G4LogicalVolume* parentVolume, 
					      G4ThreeVector parentPosition){

  // Would need to use the depthStep in the future for the creation of bounding volumes.

  // It is probably the case that the bounding box volumes can use air as their material. I
  ...
  if (turtle->children.size() == 0){
   ...
   trunkSolid = new G4Cons("Trunk",
			   pRmin1 = 0.0*m,
			   pRmax1 = (turtle->width/2.0)*m,
			   pRmin2 = 0.0*m,
			   pRmax2 = (turtle->width/2.0)*m,
			   pDz    = (turtle->length/2.0)*m,
			   pSPhi  = 0.0,
			   pDPhi  = 2.0*M_PI);
  }else{
   ...
   trunkSolid = new G4Cons("Trunk",
			   ...
			   pRmax1 = (turtle->width/2.0)*m,
			   pRmin2 = 0.0*m,
			   pRmax2 = (turtle->children[0]->width/2.0)*m,
			   pDz    = (turtle->length/2.0)*m,
			   pSPhi  = 0.0,
			   pDPhi  = 2.0*M_PI);
  }
  ...
  G4Material* trunkMaterial = MaterialFactory::instance()->getMaterial(m_trunkMaterialName);
  G4OpticalSurface* trunkOpticalSurface = MaterialFactory::instance()->getOpticalSurface(m_trunkMaterialName);

  G4LogicalVolume* trunkLogicalVolume = new G4LogicalVolume(trunkSolid, 
							    trunkMaterial,
							    "Trunk");
  ...
  rotationMatrix->set((turtle->orientation.Phi()+M_PI/2.0), 
		      turtle->orientation.Theta(),
		      0);

  TVector3 centralPosition(turtle->position);
  centralPosition = centralPosition + (turtle->orientation*(turtle->length/2.0));
  
  //G4RotationMatrix *pRot, const G4ThreeVector &tlate, G4LogicalVolume *pCurrentLogical, const G4String &pName, G4LogicalVolume *pMotherLogical, G4bool pMany, G4int pCopyNo, G4bool pSurfChk=false
  G4VPhysicalVolume* trunkPhysicalVolume = new G4PVPlacement(rotationMatrix, 
							     convertVector(centralPosition), 
							     trunkLogicalVolume, 
							     "Trunk", 
							     parentVolume,
							     false,
  ...
  //Set the surface properties
  new G4LogicalSkinSurface("TrunkSkin", trunkLogicalVolume, trunkOpticalSurface);
...
void DetectorConstruction::candidateLeafBuild() {
  ...
  for (auto& candidateLeafInfo : m_candidateLeaves) {
    ...
    bool isOverlapping  = checkForLeafOverlaps(leafLogicalVolume, trunkPhysicalVolume);
...
bool DetectorConstruction::checkForLeafOverlaps(G4LogicalVolume* candidateLeafLogicalVolume, 
  ...
  if (!parentLogicalVolume) { return false; }
