...
int main() {
  ...
  //initialConditions
  std::vector<std::shared_ptr<TreeSystemInterface> > conditions = ternaryTree->getInitialConditions();
  ...
  for ( auto& condition : conditions ){
    ...
    if ( stepCount % printFrequency == 0 ) {
      std::cout << "For step " << stepCount << " there are " << activeTurtles.size() << " active turtles and " << retiredTurtles.size() << " complete turtles." << std::endl;
    }
