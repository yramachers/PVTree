...
void showHelp() {
  ...
  std::cout << "\t --spectrumName <String>; Default 'Direct_normal_irradiance'" << std::endl;
  std::cout << "\t --outputFileName <String>; Default 'climateAffect'" << std::endl;
  std::cout << "\n\t This is quite slow, you might want to limit the date range!" << std::endl;
}
 ...
 *         the specified time periods. Simply takes the summed irradiance from the
 ...
 * @param[in] samplesPerDay The Number of spectral evaluations carried out per day.
...
TGraphAsymmErrors createSummedSpectralGraph(std::string graphName,
  ...
  std::cout << "Creating " << graphName << " graph with " << samplesPerDay << " samples per day."  << std::endl;
  ...
  for (unsigned int m=0; m<lowBinEdges.size()-1;m++){
    ...
    time_t startOfNextPeriod = lowBinEdges[m+1]-600; // (subtract ten minutes to make comparison simpler)
    ...
    while ( currentDay < startOfNextPeriod ){
      ...
      double simulationStepTime = static_cast<double>(sunsetTime-sunriseTime)/samplesPerDay;

      // Integrate over day
      for (unsigned int timeIndex=0; timeIndex<samplesPerDay; timeIndex++) {

	// Set the time to the mid-point of the day-time segment
	sun.setTime( (int)(sunriseTime + timeIndex*simulationStepTime + simulationStepTime/2.0) );

	// Get the direct normal irradiance
	double irradiance = sun.getSpectrum()->getHistogram(spectrumName)->Integral("width");
 ...
 */
TGraphAsymmErrors createRatioPlot(TGraphAsymmErrors graph1, TGraphAsymmErrors graph2) {

  TGraphAsymmErrors ratioPlot;

  if (graph1.GetN() != graph2.GetN()) {
    std::cerr << "Cannot create ratio plot as graphs have different numbers of points" << std::endl;
    return ratioPlot;
  }
...
/*! \brief Calculate the y axis range of a graph based upon the points contained.
 *
...
/*! \brief Create a canvas from a set of plots which are all overlayed on the
 *         same pad. It will save it both to the current root file and export to
 *         an eps file. The x-axis will be set to time format. In addition a ratio
 ...
 * @param[in] legendOptions List of legend draw options for each of the overlayed
 *                          plots
 *
 */
template<typename T> void createRatioCanvas(std::string canvasName,
					    std::vector<T> plots,
					    std::vector<std::string> options,
					    std::vector<int> colours,
  ...
  double absolutePaddingSize = std::fabs(maximalYValue - minimalYValue) * axisPaddingFraction;
  ...
  for (unsigned int p=0; p<plots.size(); p++) {
    ...
    if ( p == 0 ) {
      TGaxis* yAxis = new TGaxis(plots[p].GetXaxis()->GetXmin(), minimalYValue + absolutePaddingSize, 
  ...
  for (unsigned int p=0; p<plots.size(); p++){
    legend.AddEntry(&(plots[p]), legendLabels[p].c_str(), legendOptions[p].c_str());
  }
  ...
  // Add some padding to the maximum and minimum values
  absolutePaddingSize = std::fabs(maximalYValue - minimalYValue) * axisPaddingFraction;
  maximalYValue += absolutePaddingSize;
  minimalYValue -= absolutePaddingSize;
  ...
  for (unsigned int p=0; p<ratioPlots.size(); p++) {
    ...
    // Option 'S' means ticklength = fTickSize*axis_length
    if ( p == 0 ) {
      TGaxis* yAxis = new TGaxis(ratioPlots[p].GetXaxis()->GetXmin(), minimalYValue + absolutePaddingSize, 
				 ratioPlots[p].GetXaxis()->GetXmin(), maximalYValue - absolutePaddingSize,
				 minimalYValue + absolutePaddingSize, 
				 maximalYValue - absolutePaddingSize, 
				 506, "S");
      ...
      absolutePaddingSize = std::fabs(ratioPlots[p].GetXaxis()->GetXmax() 
				      - ratioPlots[p].GetXaxis()->GetXmin()) * axisPaddingFraction;

      // Option 't' for time and 'S' for ticklength as above.
      TGaxis* xAxis = new TGaxis(ratioPlots[p].GetXaxis()->GetXmin() + absolutePaddingSize, minimalYValue, 
				 ratioPlots[p].GetXaxis()->GetXmax() - absolutePaddingSize, minimalYValue,
...
int main(int argc, char** argv) {
  ...
  ops >> GetOpt::Option("spectrumName",   spectrumName,   std::string("Direct_normal_irradiance") );
  ops >> GetOpt::Option("outputFileName", outputFileName, std::string("climateAffect") );
  ...
  std::vector<time_t> monthLowBinEdges = createMonthlyBinEdges( interpretDate(startDate), interpretDate(endDate) );
  ...
  TGraphAsymmErrors clearSkyGraph = createSummedSpectralGraph("clearSkyMonthlySpectralSum",
							      spectrumName,
							      samplesPerDay, 
							      sun,
							      monthLowBinEdges);
  ...
  factory->setDefaults();
  factory->setAltitude(deviceLocation.getAltitude());
  TGraphAsymmErrors allClimateGraph = createSummedSpectralGraph("allClimateMonthlySpectralSum",
								spectrumName,
								samplesPerDay,
								sun,
								monthLowBinEdges);
  ...
  factory->setDefaults();
  factory->setAltitude(deviceLocation.getAltitude());
  TGraphAsymmErrors clearSkyNoTemperatureGraph = createSummedSpectralGraph("clearSkyNoTemperatureMonthlySpectralSum",
									   spectrumName,
									   samplesPerDay,
									   sun,
									   monthLowBinEdges);
  ...
  factory->setDefaults();
  factory->setAltitude(deviceLocation.getAltitude());
  TGraphAsymmErrors clearSkyNoPressureGraph = createSummedSpectralGraph("clearSkyNoPressureMonthlySpectralSum",
									spectrumName,
									samplesPerDay,
									sun,
									monthLowBinEdges);
  ...
  factory->setDefaults();
  factory->setAltitude(deviceLocation.getAltitude());
  TGraphAsymmErrors clearSkyNoColumnWaterGraph = createSummedSpectralGraph("clearSkyNoColumnWaterMonthlySpectralSum",
									   spectrumName,
									   samplesPerDay,
									   sun,
									   monthLowBinEdges);
  ...
  factory->setDefaults();
  factory->setAltitude(deviceLocation.getAltitude());
  TGraphAsymmErrors clearSkyNoColumnOzoneGraph = createSummedSpectralGraph("clearSkyNoColumnOzoneMonthlySpectralSum",
									   spectrumName,
									   samplesPerDay,
									   sun,
									   monthLowBinEdges);
  ...
  TGraphAsymmErrors clearSkyHeavyPollutionGraph = createSummedSpectralGraph("clearSkyHeavyPollutionMonthlySpectralSum", 
									    spectrumName,
									    samplesPerDay, 
									    sun,
									    monthLowBinEdges);
  ...
  // It takes approximately 20 mins to create all of these for a single year and 5 samples per day.
  // SMARTS is not fast :D. This is why I need to run this as a separate step during optimization.
  clearSkyGraph.Write();
  ...
  createCanvas<TGraphAsymmErrors>(outputFileName + "_allSmartsInputVariationsCanvas",
    {clearSkyGraph, clearSkyNoTemperatureGraph, clearSkyNoPressureGraph, clearSkyNoColumnWaterGraph, clearSkyNoColumnOzoneGraph, clearSkyHeavyPollutionGraph, allClimateGraph},
    {"AL", "SAMEL", "SAMEL", "SAMEL", "SAMEL", "SAMEL", "SAMEL"},
    {kBlack, kBlue-6, kGreen-5, kRed-5, kOrange+2, kMagenta-5, kRed-8},
    "Date",
    "E [kWhm^{-2}month^{-1}]"
    );

  createCanvas<TGraphAsymmErrors>(outputFileName + "_clearSkyInputVariationsCanvas",
    {clearSkyGraph, clearSkyNoTemperatureGraph, clearSkyNoPressureGraph, clearSkyNoColumnWaterGraph, clearSkyNoColumnOzoneGraph, clearSkyHeavyPollutionGraph},
  ...
  createCanvas<TGraphAsymmErrors>(outputFileName + "_clearSkyImportantInputVariationsCanvas",
    {clearSkyGraph, clearSkyNoColumnWaterGraph, clearSkyNoColumnOzoneGraph, clearSkyHeavyPollutionGraph},
    {"AL", "SAMEL", "SAMEL", "SAMEL"},
    {kBlack, kBlue-6, kGreen-5, kRed-5},
    "Date",
    "E [kWhm^{-2}month^{-1}]"
    );
  ...
  createRatioCanvas<TGraphAsymmErrors>(outputFileName + "_allSmartsInputVariationsRatioCanvas",
    {clearSkyGraph, clearSkyNoTemperatureGraph, clearSkyNoPressureGraph, clearSkyNoColumnWaterGraph, clearSkyNoColumnOzoneGraph, clearSkyHeavyPollutionGraph, allClimateGraph},
    {"APL", "SAMEPL", "SAMEPL", "SAMEPL", "SAMEPL", "SAMEPL", "SAMEPL"},
    {kBlack, kBlue-6, kGreen-5, kRed-5, kOrange+2, kMagenta-5, kRed-8},
    {20, 21, 22, 23, 33, 34, 29},
    "Date",
    "E [kWhm^{-2}month^{-1}]",
    {"nominal", "reference temperature", "reference pressure", "reference column water", "reference column ozone", "heavy pollution", "cloudy"},
    {"LP", "LP", "LP", "LP", "LP", "LP", "LP"}
    );

  createRatioCanvas<TGraphAsymmErrors>(outputFileName + "_clearSkyInputVariationsRatioCanvas",
    {clearSkyGraph, clearSkyNoTemperatureGraph, clearSkyNoPressureGraph, clearSkyNoColumnWaterGraph, clearSkyNoColumnOzoneGraph, clearSkyHeavyPollutionGraph},
    ...
    "Date",
    "E [kWhm^{-2}month^{-1}]",
    {"nominal", "reference temperature", "reference pressure", "reference column water", "reference column ozone", "heavy pollution"},
    {"LP", "LP", "LP", "LP", "LP", "LP"}
    );

  createRatioCanvas<TGraphAsymmErrors>(outputFileName + "_clearSkyImportantInputVariationsRatioCanvas",
    {clearSkyGraph, clearSkyNoColumnWaterGraph, clearSkyNoColumnOzoneGraph, clearSkyHeavyPollutionGraph},
    ...
    "Date",
    "E [kWhm^{-2}month^{-1}]",
    {"nominal", "reference column water", "reference column ozone", "heavy pollution"},
    {"LP", "LP", "LP", "LP"}
    );
