...
void addHitEfficiencyPoint(TGraphAsymmErrors& graph, 
  ...
  if (hitCountVectors.size() == 0) {
    std::cout << "Did not expect there to be no hit count vectors. Probably not a good sign!" << std::endl;
    throw new std::exception;
  }

  std::vector<long> summedEventCounts;

  for (unsigned int eventIndex=0; eventIndex<hitCountVectors[0].size(); eventIndex++) {

    long sum = 0;
    for (unsigned int countVectorIndex = 0; countVectorIndex < hitCountVectors.size(); countVectorIndex++) {
  ...
  //Effectively now the photon number in the event has gone up by a factor of hitCountVectors.size()
  ...
  for (unsigned int v=0; v< summedEventCounts.size(); v++) {
    hitSquareDifferenceSum += std::pow(averageHitNumber - summedEventCounts[v], 2.0);
  }

  double standardDeviation = std::sqrt( (1.0/(summedEventCounts.size()-1))*(hitSquareDifferenceSum) );
  ...
  graph.SetPointError(nextPointIndex, 0.0, 0.0, efficiencyStandardDeviation, efficiencyStandardDeviation);
}

/*! \brief Add the current energy density point to the graph.
 */
void addEnergyDensityPoint(TGraphAsymmErrors& graph, 
			   std::vector<std::vector<double> > summedEnergyVectors,
			   int photonNumberPerEvent, 
			   double sensitiveSurfaceArea,
  ...
  if (summedEnergyVectors.size() == 0) {
    std::cout << "Did not expect there to be no energy vectors. Probably not a good sign!" << std::endl;
    throw new std::exception;
  }

  std::vector<double> sumEnergyDensities;

  for (unsigned int eventIndex=0; eventIndex<summedEnergyVectors[0].size(); eventIndex++) {

    //Convert into Watt-seconds whilst summing (currently time steps are all same size)
    double sum = 0.0;
    for (unsigned int energyVectorIndex = 0; energyVectorIndex < summedEnergyVectors.size(); energyVectorIndex++) {
      sum += summedEnergyVectors[energyVectorIndex][eventIndex]*((double)simulationStepSize);
    }


    //Also divide by the area to get the energy density and also convert to kWhm^{-2}
    sumEnergyDensities.push_back( ( sum/sensitiveSurfaceArea) / (3600.0*1000.0) );
  }
  ...
  for (unsigned int v=0; v< sumEnergyDensities.size(); v++) {
    energySquareDifferenceSum += std::pow(averageEnergy - sumEnergyDensities[v], 2.0);
  }

  double standardDeviation = std::sqrt( (1.0/(sumEnergyDensities.size()-1))*(energySquareDifferenceSum) );

  //Set the point and 'error'.
  int nextPointIndex = graph.GetN();
  graph.SetPoint(nextPointIndex, photonNumber, averageEnergy);
  graph.SetPointError(nextPointIndex, 0.0, 0.0, standardDeviation, standardDeviation);
}
...
void wrapUp(std::vector<TGraphAsymmErrors > efficiencyGraphs, 
	    ...
	    std::vector<int> eventPhotonNumbers,
	    int simulationTimeSegments) {
  ...
  // Fill histograms of relative efficiency-error for diffent photon number trials.
  for (unsigned int p=0; p<eventPhotonNumbers.size(); p++) {

    // Create a histogram for the relative efficiency error
    std::string histName = "relativeEfficiencyErrorPhoton" + std::to_string((int)(eventPhotonNumbers[p]*simulationTimeSegments));
    TH1D relativeErrorHist(histName.c_str(), ";#DeltaEfficiency/Efficiency;N_{trees}", 100, 0.0, 1.0);
  ...
  // Fill histograms of relative surface energy density-error for different photon number trials
  for (unsigned int p=0; p<eventPhotonNumbers.size(); p++) {

    // Create a histogram for the relative error
    std::string histName = "relativeEnergyDensityErrorPhoton" + std::to_string((int)(eventPhotonNumbers[p]*simulationTimeSegments));
    TH1D relativeErrorHist(histName.c_str(), ";#DeltaE_{density}/E_density;N_{trees}", 100, 0.0, 1.0);
 ...
 *
 * Provides an example of how to integrate ROOT analysis objects into a program in
 * addition to validating that the simulation converges with a sufficient number of
 * photons.
 *
 ...
 *                 which defaults to 100. The number of time points which defaults to 10. The type of
...
int main(int argc, char** argv) {
  ...
  if (iterationNumbers.size() != eventPhotonNumbers.size()) {
    std::cout << "Number of photon numbers and number of iterations don't match." << std::endl;
    return -1;
  }
  ...
  SignalReceiver::instance()->setSignals({2, 12}, [&resultsFile,&efficiencyGraphs,&energyDensityGraphs,&eventPhotonNumbers,simulationTimeSegments](int signum) {   
      printf("Caught a signal %d\n",signum);

      // Run the standard plot creation routines with
      // whatever simulation results finished in time.
      wrapUp(efficiencyGraphs, energyDensityGraphs, eventPhotonNumbers, simulationTimeSegments);
  ...
  for (unsigned int x=0; x<treeNumber; x++){
    ...
    for (unsigned int p=0; p<eventPhotonNumbers.size(); p++) {
      ...
      ((PrimaryGeneratorAction*)runManager->GetUserPrimaryGeneratorAction())->SetPhotonNumber(eventPhotonNumbers[p]);
	...
	runManager->BeamOn(numberOfEvent);
      ...
      addHitEfficiencyPoint(effGraph,     recorder.getHitCounts(),         eventPhotonNumbers[p]);
      addEnergyDensityPoint(densityGraph, recorder.getSummedHitEnergies(), eventPhotonNumbers[p], 
			    detector->getSensitiveSurfaceArea(),
			    simulationStepTime);
    ...
    // Save the efficiency graph to the export vector and ensure y-axis range is always 0 to 1
