...
bool MaterialFactory::addConfigurationFile(std::string configurationFileName) {
  ...
  try {
    if ( m_openedConfigurations.back()->exists("extraConfiguration") ) {
      ...
      libconfig::Setting& extraConfiguration = m_openedConfigurations.back()->lookup("extraConfiguration");
    ...
    std::cerr << "No 'extra configuration' setting in configuration file." << std::endl;
  }
...
G4Material* MaterialFactory::getMaterial(std::string materialName) {
  ...
  if ( m_materialConfigurations.find(materialName) != m_materialConfigurations.end() ){
  ...
  std::cerr << "Material configuration for " << materialName << " cannot be found." << std::endl;
  return 0;
}
...
G4OpticalSurface* MaterialFactory::getOpticalSurface(std::string materialName) {
  ...
  if ( m_geant4OpticalSurfaces.find(materialName) != m_geant4OpticalSurfaces.end() ) {
  ...
  std::cerr << "Optical surface configuration for " << materialName << " cannot be found." << std::endl;
  return 0;
}


bool MaterialFactory::openConfigurationFile(std::string fileName, libconfig::Config* cfg) {
  ...
  if ( cfg->exists("material.name") ){
    ...
    if ( std::find(m_availableMaterialNames.begin(), m_availableMaterialNames.end(), materialName) != m_availableMaterialNames.end() ) {
      // Already have a material with this name.
      std::cerr << "Already have a material with name " << materialName << std::endl;
      return false;
    } else {
...
bool MaterialFactory::collectConfigurations(std::string configPath){
  ...
  if ( fileExists(configPath) ) {
    ...
    if (environmentVariableContents != 0 ){
      std::string shareFilePath(std::string(environmentVariableContents) + "/config/material/" + configPath);
...
void MaterialFactory::parseConfigurationForMaterial(std::string materialName) {
  ...
  if ( !cfg->exists("material.name") ||
       !cfg->exists("material.version") ||
       !cfg->exists("material.density") ||
       !cfg->exists("material.state") || 
       !cfg->exists("material.composition") ) {
    // Missing something important
    std::cerr << "Missing standard information in " << materialName << std::endl;
    return;
  }
  ...
  if ( cfg->exists("material.composition.baseMaterial") ) {
    ...
    std::string baseMaterialName = cfg->lookup("material.composition.baseMaterial");

    G4NistManager* man = G4NistManager::Instance();
    G4Material* baseMaterial = man->FindOrBuildMaterial(baseMaterialName);

    // Construct material based upon base
    material = new G4Material(materialName, materialDensity, baseMaterial, materialState);
    ...
    material = new G4Material(materialName, materialDensity, materialElementNumber, materialState);
    ...
    std::cerr << "Unable to parse the composition information in " << materialName << std::endl;
    return;
  }

  // Now need to interpret additional material properties (if there are any)
  if ( cfg->exists("material.defaultPhotonEnergies") && cfg->exists("material.properties") ) {

    G4MaterialPropertiesTable* materialPropertyTable = new G4MaterialPropertiesTable();

    libconfig::Setting& configEnergies = cfg->lookup("material.defaultPhotonEnergies");
    ...
    // Now go through each of the properties (adding to the material property table
    // as we go along.
...
void MaterialFactory::parseConfigurationForSurface(std::string materialName) {
  ...
  // Now need to interpret additional surface optical material properties (if there are any)
  if ( cfg->exists("material.surface.defaultPhotonEnergies") && cfg->exists("material.surface.properties") ) {

    G4MaterialPropertiesTable* materialPropertyTable = new G4MaterialPropertiesTable();

    libconfig::Setting& configEnergies = cfg->lookup("material.surface.defaultPhotonEnergies");
...
G4OpticalSurfaceFinish MaterialFactory::translateSurfaceFinish(std::string input){
...
G4OpticalSurfaceModel MaterialFactory::translateSurfaceModel(std::string input){
