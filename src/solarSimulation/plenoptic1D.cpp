#include "solarSimulation/plenoptic1D.hpp"
...
Plenoptic1D::Plenoptic1D() : m_histogram(nullptr), m_totalSurfaceFlux(0.0), m_hasSurfaceGeometry(false) {}

Plenoptic1D::Plenoptic1D(int binNumber) : m_totalSurfaceFlux(0.0), 
					  m_hasSurfaceGeometry(false) {
  m_histogram = new TH1D("plenoptic1D", "plenoptic", binNumber, 0.0, 2.0*M_PI);
  
  // I will manage the memory
  m_histogram->SetDirectory(0);
}

Plenoptic1D::Plenoptic1D(int binNumber, double minValue, double maxValue) : m_totalSurfaceFlux(0.0), 
									    m_hasSurfaceGeometry(false) {
  m_histogram = new TH1D("plenoptic1D", "plenoptic", binNumber, minValue, maxValue);

  // I will manage the memory
  m_histogram->SetDirectory(0);
}

Plenoptic1D::Plenoptic1D(std::vector<double> binLowEdges) : m_totalSurfaceFlux(0.0),
							    m_hasSurfaceGeometry(false) {

  double* binLowEdgesArray = new double[binLowEdges.size()];
  std::copy(binLowEdges.begin(), binLowEdges.end(), binLowEdgesArray);

  m_histogram = new TH1D("plenoptic1D", "plenoptic", binLowEdges.size()-1, binLowEdgesArray);
...
void Plenoptic1D::setSurfaceGeometry(std::vector<std::tuple<double, double> > vertexPositions) {
...
std::vector<std::tuple<double, double, double, double> > Plenoptic1D::generate(unsigned int number /* = 1*/) const {

  std::vector<std::tuple<double, double, double, double> > particles;
  std::vector<std::tuple<double, double, double, double, int> > particlesPerElement;
  ...
  if (!m_hasSurfaceGeometry) {
    std::cerr << "Unable to produce any particles if no surface is specified." << std::endl;
    return particles;
  }
  ...
  std::piecewise_constant_distribution<> surfaceElementDistribution( m_surfaceElementID.begin(), 
								     m_surfaceElementID.end(), 
  ...
  while (n < number) {
    ...
    if (selectedSurface < 0 || selectedSurface >= (int)(m_vertexPositions.size())-1) {
    ...
    double currentX = (std::get<0>(m_vertexPositions[selectedSurface+1])
		       - std::get<0>(m_vertexPositions[selectedSurface]) ) * fractionAlongSurface + std::get<0>(m_vertexPositions[selectedSurface]);
    double currentY = (std::get<1>(m_vertexPositions[selectedSurface+1])
		       - std::get<1>(m_vertexPositions[selectedSurface]) ) * fractionAlongSurface + std::get<1>(m_vertexPositions[selectedSurface]);

    // Choose a particle angle
    double currentAngle = m_histogram->GetRandom();

    // Count number of particles trialed
    elementParticleCounts[selectedSurface] = elementParticleCounts[selectedSurface] + 1;

    // Only use if within 90 degrees of the normal direction of the surface
    if ( deltaTheta(currentAngle, m_surfaceElementAngles[selectedSurface]) < M_PI/2.0 ){

      // Calculate weight = totalEnergy * |cos(DeltaAngle)| * length of surface / total number of particles
      double weight = totalEnergy * std::fabs( std::cos(currentAngle - m_surfaceElementAngles[selectedSurface]) )
	* m_surfaceElementLengths[selectedSurface];
      //no longer divide by number here

      // Create the tuple
      particlesPerElement.push_back( std::make_tuple(currentX, currentY, currentAngle, weight, selectedSurface) );
  ...
  for ( auto& particle : particlesPerElement ){
    ...
    particles.push_back( std::make_tuple( std::get<0>(particle),
					  std::get<1>(particle),
					  std::get<2>(particle),
...
double Plenoptic1D::calculateElementFlux(double x1, double y1, double x2, double y2) const {
  ...
  for (int b=1; b<m_histogram->GetNbinsX()+1; b++) {
    ...
    if ( deltaTheta(angle, elementAngle) < M_PI/2.0 ) {
      flux += elementLength * std::fabs(std::cos( angle - elementAngle )) * energy;
    }
  }
...
double Plenoptic1D::calculateElementLength(double x1, double y1, double x2, double y2) const {

  // Calculate the length
  double elementLength = std::sqrt( std::pow(x2-x1,2.0) + std::pow(y2-y1,2.0) );
  
  return elementLength;
}

double Plenoptic1D::calculateElementAngle(double x1, double y1, double x2, double y2) const {
