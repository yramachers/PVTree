...
std::shared_ptr<TreeConstructionInterface> TreeFactory::getTree(std::string treeName) {
  ...
  return std::shared_ptr<TreeConstructionInterface>( ((*previousEntry).second)() );
}

void TreeFactory::registerConstructor(std::string treeName, 
  ...
  if (previousEntry != m_factoryFunctionRegistry.end()) {
    throw std::invalid_argument("Cannot record two tree constructors with the same name = " + treeName);
  }
