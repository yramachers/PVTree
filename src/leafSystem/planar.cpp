...
Planar::G::G(LeafConstructionInterface* constructor, double elongation, double growthRate) : LeafSystemInterface(constructor) {
...
Planar::LeftBracket::LeftBracket(LeafConstructionInterface* constructor) : LeafSystemInterface(constructor) { }
...
void Planar::LeftBracket::processTurtles(std::vector<Turtle*>& turtleStack,
					 std::vector<Turtle*>& /*retiredTurtles*/,
					 std::vector<Polygon*>& /*leafSegments*/){
  turtleStack.push_back(new Turtle( *(turtleStack.back()) ));
}

void Planar::LeftBracket::print(std::ostream& os) const {
  os << "[";
}

//Retrieve the state from the stack
Planar::RightBracket::RightBracket(LeafConstructionInterface* constructor) : LeafSystemInterface(constructor) { }
...
void Planar::RightBracket::processTurtles(std::vector<Turtle*>& turtleStack,
					  std::vector<Turtle*>& /*retiredTurtles*/,
					  std::vector<Polygon*>& /*leafSegments*/){
...
void Planar::RightBracket::print(std::ostream& os) const {
  os << "]";
}

//Rotate around vector H by an angle in degrees.
Planar::Slash::Slash(LeafConstructionInterface* constructor, double angle) : LeafSystemInterface(constructor) {
  m_angle = angle;
}
...
Planar::Ampersand::Ampersand(LeafConstructionInterface* constructor, double angle) : LeafSystemInterface(constructor) {
  m_angle = angle;
}
...
void Planar::Ampersand::processTurtles(std::vector<Turtle*>& turtleStack,
				       std::vector<Turtle*>& /*retiredTurtles*/,
				       std::vector<Polygon*>& /*leafSegments*/){
  //Get rotation angle in radians
  double angleToRotate = m_angle * (M_PI/180.0);
...
//Start a new polygon
Planar::CurlyLeft::CurlyLeft(LeafConstructionInterface* constructor) : LeafSystemInterface(constructor) {}
...
void Planar::CurlyLeft::print(std::ostream& os) const {
  os << "{";
}

//Finish a polygon (doesn't do anything!)
Planar::CurlyRight::CurlyRight(LeafConstructionInterface* constructor) : LeafSystemInterface(constructor) {}
...
void Planar::CurlyRight::processTurtles(std::vector<Turtle*>& /*turtleStack*/,
					std::vector<Turtle*>& /*retiredTurtles*/,
					std::vector<Polygon*>& /*leafSegments*/){
  //Doesn't need to do anything just yet
}

void Planar::CurlyRight::print(std::ostream& os) const {
  os << "}";
}


//Create a vertex
Planar::Dot::Dot(LeafConstructionInterface* constructor) : LeafSystemInterface(constructor) {}
...
void Planar::Dot::print(std::ostream& os) const {
  os << ".";
}
